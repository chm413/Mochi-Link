...
class MessageValidator {
    ...
    static validateResponse(message, result) {
        ...
        if (message.success === false) {
            ...
            this.validateResponseData(message.op, message.data, result);
        }
    }
    /**
    ...
    static validateVersion(version, result) {
        ...
        else if (version !== messages_1.UWBP_VERSION) {
    ...
    static validateResponseData(op, data, result) {
        const payload = data && typeof data === 'object' ? data : {};
    ...
    static validateSystemData(op, data, result) {
        ...
        switch (op) {
            case 'handshake':
                ...
                if (payload.protocolVersion && !(0, messages_1.isCompatibleUWBPVersion)(payload.protocolVersion)) {
                    this.addError(result, 'data.protocolVersion', 'Unsupported handshake protocol version', 'INVALID_VERSION');
                }
                if (payload.capabilities !== undefined &&
                    (!Array.isArray(payload.capabilities) || payload.capabilities.some((item) => typeof item !== 'string'))) {
