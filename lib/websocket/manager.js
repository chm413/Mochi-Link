...
class WebSocketConnectionManager extends events_1.EventEmitter {
    constructor(tokenManager, config = {}, auditService // Add audit service parameter
        ...
        this.connectionLifecycles = new Map();
        this.config = {
    ...
    async handleReconnectionRequired(serverId, _reason) {
        const client = this.clients.get(serverId);
    ...
    emitLifecycleEvent(serverId, event, payload) {
        switch (event) {
            case 'connected':
    ...
    transitionLifecycle(serverId, state) {
    ...
    recordLifecycleError(serverId, payload) {
        const current = this.connectionLifecycles.get(serverId);
