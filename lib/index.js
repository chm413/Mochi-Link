...
function apply(ctx, config) {
    ...
    ctx.on('ready', async () => {
        try {
            ...
            const { SystemIntegrationService } = await Promise.resolve().then(() => __importStar(require('./services/system-integration')));
            const { HealthMonitoringService } = await Promise.resolve().then(() => __importStar(require('./services/health-monitoring')));
            const { EnvironmentDetector, DeploymentConfigManager, ConfigurationUtils } = await Promise.resolve().then(() => __importStar(require('./config/deployment')));
            // Detect environment
