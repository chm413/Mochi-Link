...
function apply(ctx, config) {
    ...
    ctx.on('ready', async () => {
        try {
            ...
            // Initialize database
            const { SimpleDatabaseManager } = require('./database/simple-init');
            dbManager = new SimpleDatabaseManager(ctx, (config.database && config.database.prefix) || 'mochi');
            await dbManager.initialize();
            logger.info('Database initialized successfully');
            isInitialized = true;
            logger.info('Mochi-Link plugin started successfully');
            logger.info('Database tables created with prefix: ' + ((config.database && config.database.prefix) || 'mochi'));
        }
        catch (error) {
