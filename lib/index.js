...
function apply(ctx, config) {
    ...
    ctx.on('ready', async () => {
        try {
            ...
            try {
                const { ServiceManager } = await Promise.resolve().then(() => __importStar(require('./services')));
                serviceManager = new ServiceManager(ctx);
                await serviceManager.initialize();
                logger.info('Service manager initialized successfully');
                // Initialize WebSocket manager if configured
                if (config.websocket?.port) {
                    const { WebSocketConnectionManager } = await Promise.resolve().then(() => __importStar(require('./websocket/manager')));
                    wsManager = new WebSocketConnectionManager(serviceManager.token, {
                        server: {
    ...
    ctx.command('mochi.server.list', '列出所有服务器')
        ...
        try {
            ...
            return '服务器列表：\n' + servers.map((s) => `  [${s.id}] ${s.name} (${s.core_type}/${s.core_name}) - ${s.status}`).join('\n');
        }
        catch (error) {
