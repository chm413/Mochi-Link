...
function apply(ctx, config) {
    ...
    ctx.command('mochi.server.add <id> <name:text>', '添加服务器')
        ...
        try {
            ...
            // 创建服务器记录
            await ctx.database.create(`${config.database?.prefix || 'mochi'}_servers`, {
                id,
                name,
            ...
            // 创建API令牌
            await ctx.database.create(`${config.database?.prefix || 'mochi'}_api_tokens`, {
                server_id: id,
                token: token,
    ...
    ctx.command('mochi.server.remove <id>', '删除服务器')
        ...
        try {
            ...
            // 删除令牌
            await ctx.database.remove(`${prefix}_api_tokens`, { server_id: id });
            // 删除ACL
            await ctx.database.remove(`${prefix}_server_acl`, { server_id: id });
            // 删除绑定
            await ctx.database.remove(`${prefix}_group_bindings`, { server_id: id });
            // 删除服务器
            await ctx.database.remove(`${prefix}_servers`, { id });
            // Create audit log using service
            if (serviceManager?.audit) {
