...
const connection_config_1 = require("./utils/connection-config");
const version_policy_1 = require("./constants/version-policy");
...
exports.Config = koishi_1.Schema.object({
    ...
    monitoring: koishi_1.Schema.object({
        reportInterval: koishi_1.Schema.number()
            .default(30)
            .min(5)
            .max(3600)
            .description('Status report interval in seconds (5s - 1h)'),
        historyRetention: koishi_1.Schema.number()
            .default(30)
            .min(1)
            .max(365)
    ...
    logging: koishi_1.Schema.object({
        ...
        auditRetention: koishi_1.Schema.number()
            .default(90)
            .min(1)
            .max(3650)
...
function apply(ctx, config) {
    ...
    const outboundClients = new Map();
    // Re-dial trigger so newly registered reverse servers connect without a reload.
    let triggerOutboundDial = null;
    const legacyModeWindows = (0, version_policy_1.getLegacyModeWindows)();
    const legacyModeWindowNotice = (0, version_policy_1.formatLegacyModeWindowNotice)(legacyModeWindows.runtimeVersion);
     ...
     * 确保未认证用户被明确拒绝
     */
    function checkAuthority(session, requiredLevel) {
    ...
    function formatConnectionModeHelp(mode) {
        if (mode === 'forward')
            return 'forward（Connector 主动连接 Koishi）';
        if (mode === 'reverse')
            return 'reverse（Koishi 主动连接 Connector）';
        return mode;
    }
    ...
    ctx.on('ready', async () => {
        try {
            ...
            table_names_1.TableNames.initialize(config.database?.prefix || 'mochi');
            ...
            try {
                ...
                const setupAuthenticatedConnection = async (connection) => {
                ...
                const routeIncomingMessage = async (message, connection) => {
                    try {
                        ...
                        switch (message.type) {
                            case 'request':
                                ...
                                const { RequestHandler } = await Promise.resolve().then(() => __importStar(require('./services/request-handler')));
                        ...
                        try {
                            ...
                            if (message.type === 'request') {
                                ...
                                const errorResponse = MessageFactory.createError(message.id, message.op, error instanceof Error ? error.message : 'Internal error', 'INTERNAL_ERROR');
                ...
                async function handleSystemMessage(message, connection) {
                    switch (message.systemOp || message.op) {
                        case 'handshake':
                            // Handle handshake message (authentication)
                            if (wsManager) {
                                const authResult = await wsManager.authManager?.handleAuthenticationMessage(message, connection.ws?.['_socket']?.remoteAddress);
                                if (authResult) {
                                    ...
                                    if (authResult.data?.success) {
                                        connection.updateCapabilities((0, capabilities_1.normalizeConnectorCapabilities)(authResult.data.capabilities));
                        ...
                        case 'capabilities':
                            if (connection.isReady()) {
                                connection.updateCapabilities((0, capabilities_1.normalizeConnectorCapabilities)(message.data?.capabilities));
                            }
                            break;
                        case 'disconnect':
                ...
                const cleanupConnection = async (connection, code, reason) => {
                ...
                wsManager.on('securityWarning', (message) => {
                    logger.warn(message);
                });
                ...
                const dialOutboundServers = async () => {
                    ...
                    let servers = [];
                    try {
                        servers = await ctx.database.get(table_names_1.TableNames.servers, {});
                    }
                    catch (error) {
                    ...
                    for (const server of servers || []) {
                        try {
                            const caps = (0, connection_config_1.resolveServerWsCapabilities)(server);
                            if (!caps.dial_outbound_ws)
                                continue;
                            if (outboundClients.has(server.id))
                                continue;
                            const parsed = (0, connection_config_1.parseConnectionConfig)(server);
                            ...
                            const { MochiWebSocketClient } = await Promise.resolve().then(() => __importStar(require('./websocket/client')));
                            ...
                            client.on('disconnected', (code, reason) => {
                                ...
                                if (connection) {
                                    ...
                                    cleanupConnection({ serverId: server.id }, code, reason);
                                }
                            });
    ...
    ctx.command('mochi.server.add <id> <name:text>', '添加服务器')
        ...
        try {
            ...
            const normalizedConnection = (0, connection_config_1.normalizeServerConnectionConfig)({}, 'forward');
            // 创建服务器记录
            await ctx.database.create(table_names_1.TableNames.servers, {
                id,
                name,
            ...
            await ctx.database.create(table_names_1.TableNames.apiTokens, {
                server_id: id,
                token: '',
    ...
    ctx.command('mochi.server.register <id> <name:text>', '注册服务器（完整信息）')
        ...
        try {
            ...
            const normalizedConnection = (0, connection_config_1.normalizeServerConnectionConfig)({
                host: host,
                port: port,
                ...(isReverse
                    ? {
    ...
    ctx.command('mochi.server.info [id]', '查看服务器信息')
        ...
        try {
            ...
            let wsCapabilities;
            try {
                wsCapabilities = (0, connection_config_1.resolveServerWsCapabilities)(server);
            }
            catch (capabilityError) {
                return `❌ 服务器连接配置冲突: ${capabilityError.message}\n` +
                    '💡 修复建议: 使用 mochi.server.update（或直接修正数据库）后，仅保留一组有效能力字段。';
            }
            const normalizedForNotice = (0, connection_config_1.normalizeServerConnectionConfig)((0, connection_config_1.parseConnectionConfig)(server), server?.connection_mode === 'forward' || server?.connection_mode === 'reverse'
            ...
            return `服务器信息：\n` +
                ...
                `  状态: ${server.status}\n` +
                `  连接模式: ${formatConnectionModeHelp((0, connection_config_1.capabilitiesToMode)(wsCapabilities))}\n` +
    ...
    ctx.command('mochi.server.token <id>', '查看令牌状态或重新生成服务器连接令牌')
        ...
        try {
            ...
            if (options.regenerate) {
                ...
                // 记录审计日志使用服务
                if (serviceManager?.audit) {
                    await serviceManager.audit.logger.logServerOperation(id, 'token.regenerate', { server_name: server.name, expires_at: expiresAt }, 'success', undefined, { userId: session?.userId });
                }
                return `✅ 令牌已重新生成\n\n` +
                    `🔐 服务器连接令牌:\n` +
                    `  服务器: ${server.name} (${id})\n` +
    ...
    ctx.command('mochi.server.remove <id>', '删除服务器')
        ...
        try {
            ...
            // 删除服务器及其所有关联数据
            // 删除令牌
            await ctx.database.remove(table_names_1.TableNames.apiTokens, { server_id: id });
            // 删除ACL
            await ctx.database.remove(table_names_1.TableNames.serverAcl, { server_id: id });
            // 删除绑定
            await ctx.database.remove(table_names_1.TableNames.groupBindings, { server_id: id });
            // 删除服务器
            await ctx.database.remove(table_names_1.TableNames.servers, { id });
            // Create audit log using service
            if (serviceManager?.audit) {
                await serviceManager.audit.logger.logServerOperation(id, 'delete', {
                    name: server.name
                }, 'success', undefined, { userId: session?.userId });
            }
            return `✅ 服务器已删除\n\n` +
    ...
    ctx.command('mochi.permission.grant <userId> <serverId> <role>', '授予权限')
        ...
        try {
            ...
            const acl = await serviceManager.permission.assignRole(userId, serverId, role, session?.userId || 'system', expiresAt);
    ...
    ctx.command('mochi.permission.revoke <userId> <serverId>', '撤销权限')
        ...
        try {
            ...
            const success = await serviceManager.permission.removeRole(userId, serverId, session?.userId || 'system');
            if (success) {
    ...
    ctx.command('mochi.permission.update <userId> <serverId> <role>', '更新权限')
        ...
        try {
            ...
            const expiresAt = options?.expires ? new Date(options.expires) : undefined;
            await serviceManager.permission.updateRole(userId, serverId, role, session?.userId || 'system', expiresAt, options?.reason);
    ...
    ctx.command('mochi.permission.list <serverId> [role]', '列出服务器权限')
        ...
        try {
            ...
            const sortedRoles = Object.keys(roleGroups).sort((a, b) => roleOrder.indexOf(a) - roleOrder.indexOf(b));
  ...
  - Admin 拥有 SM 和 PM 的所有权限`;
    ...
    ctx.command('mochi.whitelist.remove [serverId] <player:text>', '移除白名单')
        ...
        try {
            ...
            if (serviceManager?.whitelist) {
                try {
                    await serviceManager.whitelist.removeFromWhitelist(targetServerId, targetPlayer, // playerId
                    session?.userId || 'system', // executor
                    undefined // reason (可选)
                    );
                    // 记录审计日志使用服务
                    ...
                    // 记录失败的审计日志
                    if (serviceManager?.audit) {
                        await serviceManager.audit.logger.logServerOperation(targetServerId, 'whitelist.remove', { player: targetPlayer }, 'failure', error.message, { userId: session?.userId });
                    }
                    return `❌ 从白名单移除失败: ${error.message}`;
                }
            }
            else {
                return `服务器 ${server.name} 的白名单功能需要服务器连接\n` +
    ...
    ctx.command('mochi.player.kick [serverId] <player:text> [reason:text]', '踢出玩家')
        ...
        try {
            ...
            const result = await serviceManager.playerAction.kickPlayer(targetServerId, {
    ...
    ctx.command('mochi.event.types', '查看可用事件类型')
        ...
        try {
            const { SubscriptionHandler } = await Promise.resolve().then(() => __importStar(require('./services/subscription-handler')));
    ...
    ctx.command('mochi.event.subscribe [serverId] <events...>', '订阅事件')
        ...
        try {
            ...
            // 记录审计日志
            if (serviceManager.audit) {
                await serviceManager.audit.logger.logServerOperation(targetServerId, 'event.subscribe', { eventTypes, subscriptionId: subscription.id }, 'success', undefined, { userId: session?.userId });
            }
            return `✅ 事件订阅成功\n\n` +
                `📋 订阅信息：\n` +
                `  服务器: ${server.name}\n` +
    ...
    ctx.command('mochi.event.unsubscribe <subscriptionId>', '取消事件订阅')
        ...
        try {
            ...
            // 记录审计日志
            if (serviceManager.audit) {
                await serviceManager.audit.logger.logServerOperation(subscription.serverId || subscription.connectionId, 'event.unsubscribe', { subscriptionId }, 'success', undefined, { userId: session?.userId });
            }
            return `✅ 已取消订阅 ${subscriptionId}`;
        }
        catch (error) {
            logger.error('Failed to unsubscribe:', error);
            return `取消订阅失败: ${error.message}`;
        }
    });
    ...
    ctx.command('添加白名单 <player:text>', '快速添加白名单')
        ...
        try {
            ...
            if (serviceManager?.whitelist) {
                try {
                    await serviceManager.whitelist.addToWhitelist(targetServerId, player, player, session?.userId || 'system');
                    // 记录审计日志
                    if (serviceManager?.audit) {
                        await serviceManager.audit.logger.logServerOperation(targetServerId, 'whitelist.add', { player: player }, 'success', undefined, { userId: session?.userId });
                    }
                    ...
                    // 记录失败的审计日志
                    if (serviceManager?.audit) {
                        await serviceManager.audit.logger.logServerOperation(targetServerId, 'whitelist.add', { player: player }, 'failure', error.message, { userId: session?.userId });
                    }
                    return `❌ 添加到白名单失败: ${error.message}`;
                }
            }
