...
function apply(ctx, config) {
    ...
    ctx.command('mochi.whitelist.remove <serverId> <player>', '移除白名单')
        ...
        try {
            ...
            // 记录审计日志使用服务
            if (serviceManager?.audit) {
                await serviceManager.audit.logger.logServerOperation(serverId, 'whitelist.remove', { player }, 'success', undefined, { userId: session?.userId });
            }
            // TODO: 调用实际的白名单服务
    ...
    ctx.command('mochi.player.kick <serverId> <player> [reason]', '踢出玩家')
        ...
        try {
            ...
            // 记录审计日志使用服务
            if (serviceManager?.audit) {
                await serviceManager.audit.logger.logServerOperation(serverId, 'player.kick', { player, reason: reason || '无' }, 'success', undefined, { userId: session?.userId });
            }
            // TODO: 调用实际的玩家服务
        ...
        try {
            ...
            if (serviceManager?.command) {
                try {
                    ...
                    // 记录审计日志使用服务
                    if (serviceManager?.audit) {
                        await serviceManager.audit.logger.logServerOperation(serverId, 'command.execute', { command, executor: options.as }, 'success', undefined, { userId: session?.userId });
                    }
                    ...
                    // 记录失败的审计日志使用服务
                    if (serviceManager?.audit) {
                        await serviceManager.audit.logger.logServerOperation(serverId, 'command.execute', { command, executor: options.as }, 'failure', error.message, { userId: session?.userId });
                    }
