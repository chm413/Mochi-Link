    ...
    registerIntegration(integration) {
        ...
        if (this.initialized) {
            ...
            this.initializeIntegration(integration).catch(error => {
                if (this.logger) {
                    this.logger.error(`Failed to initialize plugin integration ${integration.type}:`, error);
                }
            });
        }
    }
    ...
    async refreshAvailability() {
        const checkPromises = Array.from(this.integrations.values()).map(async (integration) => {
            try {
                ...
                if (this.logger) {
                    this.logger.error(`Failed to check availability for ${integration.type}:`, error);
                }
            }
        });
    ...
    async cleanup() {
        const cleanupPromises = Array.from(this.integrations.values()).map(async (integration) => {
            try {
                ...
                if (this.logger) {
                    this.logger.error(`Failed to cleanup plugin integration ${integration.type}:`, error);
                }
            }
        });
