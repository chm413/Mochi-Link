...
class DatabaseQueryOptimizer {
    ...
    async getUserPermissionsOptimized(userId) {
        ...
        const acls = await this.ctx.database.get(table_names_1.TableNames.serverAcl, {
            user_id: userId,
            $or: [
                { expires_at: null },
    ...
    async getAuditLogsOptimized(filters) {
        ...
        const [logs, totalCount] = await Promise.all([
            this.ctx.database.get(table_names_1.TableNames.auditLogs, query, {
                limit: filters.limit || 100,
                offset: filters.offset || 0
            }),
            this.ctx.database.get(table_names_1.TableNames.auditLogs, query, ['id']).then(results => results.length)
        ]);
        const executionTime = Date.now() - startTime;
    ...
    async optimizePendingOperations(serverId) {
        ...
        const operations = await this.ctx.database.get(table_names_1.TableNames.pendingOperations, {
            server_id: serverId,
            status: 'pending'
        });
    ...
    async warmUpCache() {
        ...
        try {
            ...
            const recentServers = await this.ctx.database.get(table_names_1.TableNames.minecraftServers, {
            ...
            const recentAcls = await this.ctx.database.get(table_names_1.TableNames.serverAcl, {
                $or: [
                    { expires_at: null },
                    { expires_at: { $gt: new Date() } }
                ]
