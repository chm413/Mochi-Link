...
class TokenOperations {
    ...
    async createToken(serverId, token, tokenHash, ipWhitelist, encryptionConfig, expiresAt) {
        ...
        const created = await this.getTokenById(result.id);
        if (!created) {
...
class PendingOperationsManager {
    ...
    async addOperation(serverId, operationType, target, parameters, scheduledAt) {
        ...
        // Koishi v4: database.create 返回创建的行对象（非数组）
        const created = await this.getOperation(result.id);
        if (!created) {
...
class DatabaseManager {
     ...
     * 确保服务器创建和令牌生成是原子操作
     */
    async createServerWithToken(serverConfig, token, tokenHash, ipWhitelist, encryptionConfig, expiresAt) {
        ...
        try {
            ...
            serverCreated = true;
            // Step 2: Create token
            const apiToken = await this.tokens.createToken(serverConfig.id, token, tokenHash, ipWhitelist, encryptionConfig, expiresAt);
            tokenCreated = true;
     ...
     * 确保权限授予和审计日志记录是原子操作
     */
    async grantPermissionWithAudit(userId, serverId, role, permissions, grantedBy, auditContext, expiresAt) {
        ...
        try {
            ...
            aclCreated = true;
            // Step 2: Log audit
            await this.audit.logOperation(grantedBy, serverId, 'permission.grant', { userId, role, permissions }, 'success', undefined, auditContext?.ipAddress, auditContext?.userAgent);
            return acl;
        }
        catch (error) {
            // Rollback on error
     ...
     */
    async revokePermissionWithAudit(userId, serverId, revokedBy, auditContext) {
        ...
        try {
            ...
            // Step 2: Log audit
            await this.audit.logOperation(revokedBy, serverId, 'permission.revoke', { userId }, 'success', undefined, auditContext?.ipAddress, auditContext?.userAgent);
            return revoked;
        }
        catch (error) {
            // Rollback on error
            if (revoked && existingACL) {
                try {
                    await this.acl.grantPermission(userId, serverId, existingACL.role, existingACL.permissions, existingACL.grantedBy, existingACL.expiresAt);
     ...
     */
    async deleteServerWithRelations(serverId) {
        ...
        try {
            ...
            const bindingsResult = await this.ctx.database.remove(table_names_1.TableNames.serverBindings, {
                server_id: serverId
            });
