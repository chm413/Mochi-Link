...
class ServerOperations {
    ...
    async createServer(config) {
        ...
        await this.ctx.database.create('minecraft_servers', dbServer);
    ...
    async getServer(serverId) {
        const dbServers = await this.ctx.database.get('minecraft_servers', { id: serverId });
     ...
     */
    async getAllServers() {
        const dbServers = await this.ctx.database.get('minecraft_servers', {});
    ...
    async getServersByOwner(ownerId) {
        const dbServers = await this.ctx.database.get('minecraft_servers', { owner_id: ownerId });
        return dbServers.map(models_1.ModelUtils.dbServerToModel);
    }
    /**
    ...
    async updateServer(serverId, updates) {
        ...
        await this.ctx.database.set('minecraft_servers', { id: serverId }, dbUpdates);
    ...
    async updateServerStatus(serverId, status, lastSeen) {
        ...
        await this.ctx.database.set('minecraft_servers', { id: serverId }, updates);
    }
    /**
     * Delete server
     */
    async deleteServer(serverId) {
        const result = await this.ctx.database.remove('minecraft_servers', { id: serverId });
...
class ACLOperations {
    ...
    async grantPermission(userId, serverId, role, permissions, grantedBy, expiresAt) {
        ...
        await this.ctx.database.remove('server_acl', {
            user_id: userId,
            server_id: serverId
        });
        // Create new ACL
        const result = await this.ctx.database.create('server_acl', dbACL);
    ...
    async getACL(userId, serverId) {
        const dbACLs = await this.ctx.database.get('server_acl', {
            user_id: userId,
            server_id: serverId
        });
        if (dbACLs.length === 0) {
    ...
    async getServerACLs(serverId) {
        const dbACLs = await this.ctx.database.get('server_acl', { server_id: serverId });
    ...
    async getUserACLs(userId) {
        const dbACLs = await this.ctx.database.get('server_acl', { user_id: userId });
        return dbACLs.map(models_1.ModelUtils.dbACLToModel);
    }
    /**
    ...
    async revokePermission(userId, serverId) {
        const result = await this.ctx.database.remove('server_acl', {
            user_id: userId,
            server_id: serverId
        });
        return result.matched ? result.matched > 0 : false;
...
class TokenOperations {
    ...
    async createToken(serverId, token, tokenHash, ipWhitelist, encryptionConfig, expiresAt) {
        ...
        const result = await this.ctx.database.create('api_tokens', dbToken);
    ...
    async getTokenById(tokenId) {
        const dbTokens = await this.ctx.database.get('api_tokens', { id: tokenId });
    ...
    async getTokenByHash(tokenHash) {
        const dbTokens = await this.ctx.database.get('api_tokens', { token_hash: tokenHash });
        if (dbTokens.length === 0) {
            return null;
        }
        return this.dbTokenToModel(dbTokens[0]);
    ...
    async getServerTokens(serverId) {
        const dbTokens = await this.ctx.database.get('api_tokens', { server_id: serverId });
    ...
    async updateTokenLastUsed(tokenId) {
        await this.ctx.database.set('api_tokens', { id: tokenId }, {
            last_used: new Date()
        });
    }
    /**
    ...
    async deleteToken(tokenId) {
        const result = await this.ctx.database.remove('api_tokens', { id: tokenId });
        return result.matched ? result.matched > 0 : false;
    }
    /**
    ...
    async cleanupExpiredTokens() {
        const now = new Date();
        const result = await this.ctx.database.remove('api_tokens', {
            expires_at: { $lt: now }
        });
        return result.matched || 0;
    }
...
class AuditOperations {
    ...
    async logOperation(userId, serverId, operation, operationData, result, errorMessage, ipAddress, userAgent) {
        ...
        const insertResult = await this.ctx.database.create('audit_logs', dbAudit);
    ...
    async getAuditLog(logId) {
        const dbLogs = await this.ctx.database.get('audit_logs', { id: logId });
        if (dbLogs.length === 0) {
            return null;
        }
    ...
    async getAuditLogs(filters) {
        ...
        const dbLogs = await this.ctx.database.get('audit_logs', query, {
            limit: filters.limit || 100,
            offset: filters.offset || 0
        });
        return dbLogs.map(models_1.ModelUtils.dbAuditToModel);
    }
    /**
    ...
    async cleanupOldLogs(retentionDays) {
        ...
        const result = await this.ctx.database.remove('audit_logs', {
...
class PendingOperationsManager {
    ...
    async addOperation(serverId, operationType, target, parameters, scheduledAt) {
        ...
        const result = await this.ctx.database.create('pending_operations', dbOperation);
    ...
    async getOperation(operationId) {
        const dbOps = await this.ctx.database.get('pending_operations', { id: operationId });
        if (dbOps.length === 0) {
            return null;
        }
    ...
    async getPendingOperations(serverId) {
        const dbOps = await this.ctx.database.get('pending_operations', {
            server_id: serverId,
            status: 'pending'
        });
    ...
    async updateOperationStatus(operationId, status, executedAt, error) {
        ...
        await this.ctx.database.set('pending_operations', { id: operationId }, updates);
    }
    /**
    ...
    async optimizeOperations(serverId) {
        ...
        for (const [target, targetOps] of Array.from(operationsByTarget.entries())) {
            ...
            for (const add of adds) {
                ...
                if (matchingRemove) {
                    await this.ctx.database.remove('pending_operations', { id: add.id });
                    await this.ctx.database.remove('pending_operations', { id: matchingRemove.id });
                    optimizedCount += 2;
                }
            }
        }
...
class DatabaseManager {
    ...
    async healthCheck() {
        try {
            ...
            await this.ctx.database.get('minecraft_servers', {}, ['id']);
            return true;
        }
        catch (error) {
    ...
    async getStatistics() {
        const [servers, acls, tokens, auditLogs, pendingOps] = await Promise.all([
            this.ctx.database.get('minecraft_servers', {}, ['id']),
            this.ctx.database.get('server_acl', {}, ['id']),
            this.ctx.database.get('api_tokens', {}, ['id']),
            this.ctx.database.get('audit_logs', {}, ['id']),
            this.ctx.database.get('pending_operations', {}, ['id'])
        ]);
        return {
