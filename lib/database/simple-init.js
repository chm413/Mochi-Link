...
class SimpleDatabaseManager {
    ...
    async initialize() {
        ...
        ctx.model.extend(`${prefix}pending_operations`, {
            id: 'unsigned',
            server_id: 'string',
        ...
        // Player Cache Table
        ctx.model.extend(`${prefix}player_cache`, {
            id: 'unsigned',
            uuid: 'string',
            xuid: 'string',
            name: 'string',
        ...
        ctx.model.extend(`${prefix}server_bindings`, {
            id: 'unsigned',
            group_id: 'string',
            server_id: 'string',
            binding_type: 'string',
    ...
    async createAPIToken(serverId, token, tokenHash, options) {
        ...
        const result = await this.ctx.database.create(`${prefix}api_tokens`, tokenData);
        return {
            id: result.id,
            serverId: serverId,
            token: token,
    ...
    async deleteAPIToken(tokenId) {
        const prefix = this.tablePrefix.replace(/\.$/, '_');
        await this.ctx.database.remove(`${prefix}api_tokens`, { id: tokenId });
    }
    /**
    ...
    async deleteServerAPITokens(serverId) {
        const prefix = this.tablePrefix.replace(/\.$/, '_');
        await this.ctx.database.remove(`${prefix}api_tokens`, { server_id: serverId });
    }
}
