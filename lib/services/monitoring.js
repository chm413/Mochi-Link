...
class MonitoringService {
    constructor(ctx, auditService, eventService, config, bridgeGetter) {
        this.ctx = ctx;
        this.auditService = auditService;
        this.eventService = eventService;
        this.bridgeGetter = bridgeGetter;
        this.reportIntervals = new Map();
     ...
     */
    async registerServer(serverId, connection) {
        ...
        await this.auditService.logger.logSuccess('monitoring.register_server', { serverId }, { serverId });
    ...
    async unregisterServer(serverId) {
        ...
        await this.auditService.logger.logSuccess('monitoring.unregister_server', { serverId }, { serverId });
    ...
    async updateConfig(newConfig) {
        ...
        await this.auditService.logger.logSuccess('monitoring.update_config', { oldConfig, newConfig: this.config });
     ...
     */
    async collectAndReportStatus(serverId) {
     ...
     * Request server info from connector
     */
    async requestServerInfo(connection) {
        const bridge = this.bridgeGetter?.(connection.serverId) ?? connection.bridge;
     ...
     */
    async requestPerformanceMetrics(connection) {
        const bridge = this.bridgeGetter?.(connection.serverId) ?? connection.bridge;
        if (!bridge || !bridge.isConnectedToBridge?.()) {
            throw new Error(`Connector is unavailable for ${connection.serverId}`);
        }
     ...
     */
    determineServerStatus(serverInfo, metrics) {
     ...
     * Store historical data
     */
    async storeHistoricalData(report) {
     ...
     * Get historical data
     */
    async getHistoricalData(serverId, timeRange, interval = 1) {
        const history = this.historicalData.get(serverId) || [];
        // Filter by time range
        const filteredHistory = history.filter(metric => metric.timestamp >= timeRange.start.getTime() &&
     ...
     */
    aggregateHistoricalData(metrics, intervalMinutes) {
    ...
    async checkAlerts(report) {
        ...
        for (const alertData of alerts) {
            await this.sendAlert(report.serverId, alertData.type, alertData.severity, alertData.message, alertData.data);
        }
    }
    /**
     * Send alert
     */
    async sendAlert(serverId, type, severity, message, data) {
        ...
        // Log alert
        await this.auditService.logger.logSuccess('monitoring.alert_generated', alert, { serverId });
    ...
    async acknowledgeAlert(alertId, userId) {
        ...
        await this.auditService.logger.logSuccess('monitoring.alert_acknowledged', { alertId, acknowledgedAt: alert.acknowledgedAt }, { userId, serverId: alert.serverId });
        this.logger.info(`Alert acknowledged: ${alertId} by ${userId}`);
    }
    /**
     * Get active alerts
     */
    getActiveAlerts(serverId) {
     ...
     */
    async getServerPerformanceSummary(serverId) {
        const history = this.historicalData.get(serverId) || [];
