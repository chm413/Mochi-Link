...
class ServerControlService {
    ...
    async restartServer(serverId, options = {}) {
        ...
        try {
            // Permission check
            if (options.executor) {
                const hasPermission = await this.permissionManager.checkPermission(options.executor, serverId, 'server.restart');
            ...
            // Audit log
            await this.auditService.logger.logSuccess('server.restart', {
                serverId,
                delay,
                message,
            ...
            // Audit log
            await this.auditService.logger.logError('server.restart', {
                serverId,
                duration: Date.now() - startTime
     ...
     * Shutdown a server
     */
    async shutdownServer(serverId, options = {}) {
        const startTime = Date.now();
        try {
            // Permission check
            if (options.executor) {
                const hasPermission = await this.permissionManager.checkPermission(options.executor, serverId, 'server.shutdown');
                if (!hasPermission.granted) {
            ...
            // Audit log
            await this.auditService.logger.logSuccess('server.shutdown', {
                serverId,
                delay,
                message,
                duration: Date.now() - startTime
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            // Audit log
            await this.auditService.logger.logError('server.shutdown', {
                serverId,
                duration: Date.now() - startTime
            }, errorMessage, { userId: options.executor });
     ...
     * Save server worlds
     */
    async saveServer(serverId, options = {}) {
        const startTime = Date.now();
        try {
            // Permission check
            if (options.executor) {
                const hasPermission = await this.permissionManager.checkPermission(options.executor, serverId, 'server.save');
                if (!hasPermission.granted) {
            ...
            // Audit log
            await this.auditService.logger.logSuccess('server.save', {
                serverId,
                worlds,
                duration
            }, { userId: options.executor });
            ...
            const duration = Date.now() - startTime;
            // Audit log
            await this.auditService.logger.logError('server.save', {
                serverId,
                duration
            }, errorMessage, { userId: options.executor });
     ...
     */
    async reloadServer(serverId, options = {}) {
        const startTime = Date.now();
        try {
            // Permission check
            if (options.executor) {
                const hasPermission = await this.permissionManager.checkPermission(options.executor, serverId, 'server.reload');
                if (!hasPermission.granted) {
            ...
            const duration = Date.now() - startTime;
            // Audit log
            await this.auditService.logger.logSuccess('server.reload', {
                serverId,
                type: reloadType,
                duration
            }, { userId: options.executor });
            ...
            const duration = Date.now() - startTime;
            // Audit log
            await this.auditService.logger.logError('server.reload', {
                serverId,
                duration
            }, errorMessage, { userId: options.executor });
     ...
     */
    async restartServers(serverIds, options = {}) {
        const results = [];
     ...
     */
    async saveServers(serverIds, options = {}) {
        const results = [];
        for (const serverId of serverIds) {
