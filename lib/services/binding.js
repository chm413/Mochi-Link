...
class BindingManager {
    ...
    async createBinding(userId, options) {
        ...
        const servers = await this.ctx.database.get(table_names_1.TableNames.minecraftServers, { id: options.serverId });
        if (servers.length === 0) {
        ...
        const existingGroupBindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, {
            group_id: options.groupId
        });
        if (existingGroupBindings.length > 0) {
            const existingBinding = existingGroupBindings[0];
            const existingServer = await this.ctx.database.get(table_names_1.TableNames.minecraftServers, {
                id: existingBinding.server_id
            });
            const serverName = existingServer.length > 0 ? existingServer[0].name : existingBinding.server_id;
            throw new types_1.MochiLinkError(`This group is already bound to server "${serverName}" (${existingBinding.server_id}). ` +
                `Please remove the existing binding first using: mochi.bind.remove ${existingBinding.id}`, 'GROUP_ALREADY_BOUND');
        }
        ...
        await this.ctx.database.create(table_names_1.TableNames.serverBindings, bindingData);
        // Get the created binding (since create doesn't return the created record)
        const createdBindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, {
            group_id: options.groupId,
            server_id: options.serverId,
            binding_type: options.bindingType
    ...
    async updateBinding(userId, bindingId, options) {
        ...
        const existingBindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, { id: bindingId });
        ...
        await this.ctx.database.set(table_names_1.TableNames.serverBindings, bindingId, updateData);
        // Get updated binding
        const [updated] = await this.ctx.database.get(table_names_1.TableNames.serverBindings, bindingId);
    ...
    async deleteBinding(userId, bindingId) {
        ...
        // Delete binding
        await this.ctx.database.remove(table_names_1.TableNames.serverBindings, bindingId);
        // Clear caches
        this.clearCaches();
        // Audit log
    ...
    async getBinding(bindingId) {
        ...
        const bindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, { id: bindingId });
    ...
    async queryBindings(query) {
        ...
        // Get total count
        const allBindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, conditions);
        const total = allBindings.length;
        // Get bindings with pagination
        const dbBindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, conditions, {
    ...
    async getGroupRoutes(groupId) {
        ...
        const bindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, { group_id: groupId });
    ...
    async getGroupServers(groupId, bindingType) {
        ...
        const bindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, conditions);
    ...
    async updateBindingActivity(groupId, serverId, bindingType) {
        const binding = await this.ctx.database.get(table_names_1.TableNames.serverBindings, {
            group_id: groupId,
            server_id: serverId,
    ...
    async monitorBindingHealth() {
        ...
        const allBindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, {});
        const now = Date.now();
    ...
    async deleteServerBindings(userId, serverId) {
        ...
        const bindings = await this.ctx.database.get(table_names_1.TableNames.serverBindings, { server_id: serverId });
        let deletedCount = 0;
        for (const binding of bindings) {
            try {
                await this.deleteBinding(userId, binding.id);
    ...
    async getHealthStatus() {
        try {
            ...
            await this.ctx.database.get(table_names_1.TableNames.serverBindings, {}, { limit: 1 });
