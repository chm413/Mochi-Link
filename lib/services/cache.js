...
class CacheService extends events_1.EventEmitter {
    ...
    async preloadUserPermissions() {
        try {
            ...
            const activeACLs = await this.ctx.database.get(table_names_1.TableNames.serverAcl, {
                $or: [
                    { expires_at: null },
            ...
            for (const [userId, permissions] of userPermissions) {
                if (count >= (this.config.preloadBatchSize || 50))
                    break;
    ...
    async preloadPlayerInfo() {
        try {
            ...
            const recentPlayers = await this.ctx.database.get(table_names_1.TableNames.playerCache, {
