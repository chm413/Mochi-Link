...
class TokenManager {
    ...
    async generateToken(options, createdBy) {
        try {
            ...
            const token = await this.tokenOps.createToken(options.serverId, rawToken, // Returned once to the caller; TokenOperations stores only the hash
    ...
    async refreshToken(tokenId, options, refreshedBy) {
        try {
            ...
            // Create new token
            const refreshedToken = await this.tokenOps.createToken(existingToken.serverId, newRawToken, // Returned once to this operation; persistence stores only the hash
