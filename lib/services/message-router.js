...
class MessageRouter extends events_1.EventEmitter {
    ...
    formatMessage(content, message, format) {
        // 清理用户输入，防止 XSS
        const safeContent = (0, security_1.sanitizeUserInput)(content, { maxLength: 2000, allowNewlines: true });
        const safeUsername = (0, security_1.sanitizeUserInput)(message.userName, { maxLength: 32 });
        const safeGroupId = (0, security_1.sanitizeUserInput)(message.groupId, { maxLength: 64 });
        if (!format) {
            return `[${safeUsername}] ${safeContent}`;
        }
        // 验证消息格式模板
        const validation = (0, security_1.validateMessageFormat)(format);
        if (!validation.valid) {
    ...
    formatEventMessage(event, format) {
        // 清理服务器 ID
        const safeServerId = (0, security_1.sanitizeUserInput)(event.serverId, { maxLength: 64 });
        const safeEventType = (0, security_1.sanitizeUserInput)(event.eventType, { maxLength: 64 });
        if (!format) {
        ...
        const dataPlaceholders = event.data && typeof event.data === 'object'
            ...
            : [];
        const validation = (0, security_1.validateMessageFormat)(format, dataPlaceholders);
        if (!validation.valid) {
        ...
        if (event.data && typeof event.data === 'object') {
            Object.keys(event.data).forEach(key => {
                ...
                const safeValue = (0, security_1.sanitizeUserInput)(value, { maxLength: 200 });
