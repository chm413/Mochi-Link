...
const optimization_1 = require("../database/optimization");
...
class PerformanceOptimizationService extends events_1.EventEmitter {
    ...
    initializeComponents() {
        ...
        this.dbOptimizer = new optimization_1.DatabaseQueryOptimizer(this.ctx, this.dbManager, {
        ...
        this.connectionPool = new pool_1.ConnectionPool(this.ctx, {
        ...
        this.cacheService = new cache_1.CacheService(this.ctx, {
        ...
        this.resourceManager = new pool_1.ResourceManager(this.ctx);
    ...
    async getUserPermissionsOptimized(userId) {
        // Try cache first
        const cached = await this.cacheService.get(`permissions:${userId}`);
        if (cached) {
            return cached;
        }
        // Use optimized database query
     ...
     */
    calculateOverallHealth(dbStats, poolStats, cacheStats) {
     ...
     */
    calculatePerformanceScore(dbStats, poolStats, cacheStats) {
        let score = 100;
     ...
     */
    generateRecommendations(dbStats, poolStats, cacheStats) {
