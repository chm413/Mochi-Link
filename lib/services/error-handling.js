...
exports.ErrorHandlingService = exports.ConnectionQualityMonitor = exports.ExponentialBackoffManager = void 0;
const events_1 = require("events");
const business_error_handler_1 = require("./business-error-handler");
// ============================================================================
...
class ExponentialBackoffManager {
    ...
    calculateDelay(key) {
        ...
        let delay = Math.min(baseDelay * Math.pow(multiplier, attempts), this.config.maxRetryInterval);
...
// ============================================================================
class ErrorHandlingService extends events_1.EventEmitter {
    constructor(ctx, auditService, config = {}) {
        ...
        this.businessErrorHandler = new business_error_handler_1.BusinessErrorHandler(ctx, auditService);
     ...
     */
    async handleConnectionFailure(serverId, error, serverConfig) {
        ...
        // Log the error
        await this.auditService.logger.logError('connection_failed', {
            error: error.message,
     ...
     */
    async handleAuthenticationFailure(serverId, error, reason) {
        const logger = this.ctx.logger('mochi-link:error-handler');
     ...
     */
    async handleProtocolError(serverId, error, message) {
        const logger = this.ctx.logger('mochi-link:error-handler');
        // Create error context
        ...
        context.metadata.lastError = error.message;
        // Log protocol error
        await this.auditService.logger.logError('protocol_error', {
            error: error.message,
     ...
     */
    async handleSyncConflict(serverId, conflictType, conflictData) {
        const conflict = {
            type: conflictType,
            serverId,
            conflictData,
     ...
     * Set up business error handler event forwarding
     */
    setupBusinessErrorHandlerEvents() {
     ...
     */
    async scheduleRecovery(serverId, context, serverConfig) {
     ...
     */
    async attemptFailover(serverId, serverConfig) {
     ...
     */
    async handleMaxAttemptsReached(serverId, context, serverConfig) {
        const logger = this.ctx.logger('mochi-link:error-handler');
    ...
    // Utility Methods
    // ============================================================================
    getOrCreateErrorContext(key, category, severity) {
     ...
     */
    recordConnectionSuccess(serverId, latency) {
