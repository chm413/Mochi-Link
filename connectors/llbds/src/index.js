...
class MochiLinkLLBDSPlugin {
    ...
    async checkExternalService() {
        try {
            const response = await fetchWithTimeout(`http://localhost:${this.config.getExternalServicePort()}/health`, {}, 5000);
            return response.ok;
        }
        catch (error) {
            return false;
    ...
    async cleanup() {
        try {
            ...
            try {
                await fetchWithTimeout(`http://localhost:${this.config.getExternalServicePort()}/shutdown`, { method: 'POST' }, 5000);
            }
            catch (error) {
