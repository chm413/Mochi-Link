package com.mochilink.connector.fabric.commands;
...
/**
 * MochiLink Fabric Command
 * 
 * Command handler for Fabric mod.
 * 
 * @author chm413
 * @version 2.1.0
 */
public class MochiLinkFabricCommand {
    
    private final MochiLinkFabricMod mod;
    
    public MochiLinkFabricCommand(MochiLinkFabricMod mod) {
        this.mod = mod;
    }
    ...
    public void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(literal("mochilink")
            ...
            .then(literal("config")
                ...
                .then(literal("set")
                    .executes(ctx -> {
                        sendMessage(ctx, "§cUsage: /mochilink config set <key> <value>");
                        return 1;
                    })))
            .then(literal("reload")
     ...
     */
    private int handleStatus(CommandContext<ServerCommandSource> ctx) {
        boolean connected = mod.isConnectedToManagement();
        String status = mod.getConnectionStatus();
        
        sendMessage(ctx, "§6=== MochiLink Fabric Status ===");
     ...
     */
    private int handleReconnect(CommandContext<ServerCommandSource> ctx) {
     ...
     * Handle info command
     */
    private int handleInfo(CommandContext<ServerCommandSource> ctx) {
        sendMessage(ctx, "§6=== MochiLink Fabric Info ===");
        sendMessage(ctx, "§eVersion: §f2.1.0");
        sendMessage(ctx, "§eProtocol: §fU-WBP v2.0");
        sendMessage(ctx, "§eServer Type: §fFabric");
     ...
     */
    private int handleStats(CommandContext<ServerCommandSource> ctx) {
     ...
     */
    private int handleConfigGetAll(CommandContext<ServerCommandSource> ctx) {
        if (mod.getConfig() == null) {
     ...
     */
    private int handleSubscriptions(CommandContext<ServerCommandSource> ctx) {
     ...
     */
    private int handleReconnectionStatus(CommandContext<ServerCommandSource> ctx) {
        if (mod.getConnectionManager() == null) {
     ...
     */
    private int handleReconnectionEnable(CommandContext<ServerCommandSource> ctx) {
        if (mod.getConnectionManager() == null) {
            sendMessage(ctx, "§cConnection manager not available");
            return 0;
        }
     ...
     */
    private int handleReconnectionDisable(CommandContext<ServerCommandSource> ctx) {
        if (mod.getConnectionManager() == null) {
            sendMessage(ctx, "§cConnection manager not available");
            return 0;
        }
     ...
     * Handle help command
     */
    private int handleHelp(CommandContext<ServerCommandSource> ctx) {
        sendMessage(ctx, "§6=== MochiLink Fabric Commands ===");
     ...
     */
    private void sendMessage(CommandContext<ServerCommandSource> ctx, String message) {
