# Mochi-Link (大福连) - 项目介绍与部署指南

[![npm](https://img.shields.io/npm/v/koishi-plugin-mochi-link)](https://www.npmjs.com/package/koishi-plugin-mochi-link)
[![License](https://img.shields.io/github/license/your-org/koishi-plugin-mochi-link)](https://github.com/your-org/koishi-plugin-mochi-link/blob/main/LICENSE)
[![Build Status](https://img.shields.io/github/workflow/status/your-org/koishi-plugin-mochi-link/CI)](https://github.com/your-org/koishi-plugin-mochi-link/actions)

## 📖 项目简介

Mochi-Link（大福连）是一个基于 Koishi 框架的 **Minecraft 统一管理与监控系统**。如同大福（麻薯）一样具有极强的黏性和包容性，把各种核心（LLBDS、Paper、Folia 等）软糯地包裹在一起，创建一个"全核心统一"的 MC 服务器管理层。

### 🎯 核心特性

#### 🌐 跨核心统一接口
- **Java 版服务器支持**：Paper、Folia、Fabric、Forge、Mohist、Geyser
- **基岩版服务器支持**：LLBDS、Nukkit、PMMP、BDS
- **统一协议**：通过标准化的 U-WBP v2 协议统一管理不同核心

#### 🔗 灵活的连接架构
- **正向连接**：Koishi 插件作为客户端连接到服务器
- **反向连接**：服务器作为客户端连接到 Koishi 插件
- **多种接入模式**：插件、RCON、终端注入

#### 👥 多服务器管理
- 单个 Koishi 实例管理多台 MC 服务器
- 群聊与服务器的多对多绑定关系
- 基于服务器 ID 的权限分离控制

#### 🛡️ 完善的安全机制
- API 令牌认证和 IP 白名单
- 可选的 AES/RSA 通信加密
- 基于角色的权限管理系统
- 完整的审计日志记录

#### 📊 实时监控推送
- **玩家事件**：加入、离开、聊天、死亡、成就
- **服务器状态**：在线状态、TPS、内存使用
- **性能告警**：TPS 过低、内存过高、玩家洪水攻击

#### 🎮 统一玩家管理
- 跨服务器玩家信息聚合
- 非正版玩家身份识别
- 统一的黑白名单和封禁系统
- 离线操作缓存和同步

## 🏗️ 系统架构

### 架构图

```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   外部管理系统   │    │   Web 管理面板   │    │   Koishi 机器人  │
└─────────┬───────┘    └─────────┬───────┘    └─────────┬───────┘
          │                      │                      │
          └──────────────────────┼──────────────────────┘
                                 │
                    ┌─────────────┴─────────────┐
                    │     Mochi-Link 插件      │
                    │  ┌─────────┬─────────┐   │
                    │  │ HTTP API│WebSocket│   │
                    │  │ Server  │ Server  │   │
                    │  └─────────┴─────────┘   │
                    │  ┌─────────────────────┐ │
                    │  │   Core Services     │ │
                    │  │ - Protocol Handler  │ │
                    │  │ - Connection Mgr    │ │
                    │  │ - Permission Mgr    │ │
                    │  │ - Data Synchronizer │ │
                    │  └─────────────────────┘ │
                    │  ┌─────────────────────┐ │
                    │  │   Koishi Database   │ │
                    │  └─────────────────────┘ │
                    └─────────────┬─────────────┘
                                  │
                    ┌─────────────┴─────────────┐
                    │    Connector Bridges     │
                    └─────────────┬─────────────┘
                                  │
        ┌─────────────────────────┼─────────────────────────┐
        │                         │                         │
┌───────┴───────┐        ┌────────┴────────┐       ┌────────┴────────┐
│  Java 服务器   │        │   基岩版服务器   │       │   其他核心服务器  │
│ Paper/Folia   │        │  LLBDS/PMMP    │       │ Fabric/Forge    │
└───────────────┘        └─────────────────┘       └─────────────────┘
```

### 技术栈

- **后端框架**：Koishi v4.15+
- **开发语言**：TypeScript 5.0+
- **数据库**：支持 Koishi 所有数据库适配器
- **通信协议**：WebSocket (U-WBP v2)、HTTP REST API
- **测试框架**：Jest + fast-check (属性测试)
- **构建工具**：TypeScript Compiler
- **部署工具**：PM2、Docker (可选)

## 📋 系统要求

### 最低要求
- **Node.js**: 16.0+ (推荐 18.0+)
- **内存**: 512MB RAM (推荐 2GB+)
- **存储**: 100MB 可用空间
- **网络**: 支持 WebSocket 连接

### 推荐配置
- **Node.js**: 18.0+ LTS
- **内存**: 4GB+ RAM
- **存储**: 1GB+ 可用空间
- **CPU**: 2+ 核心
- **网络**: 稳定的网络连接，低延迟

### 支持的操作系统
- **Linux**: Ubuntu 18.04+, CentOS 7+, Debian 9+
- **Windows**: Windows 10+, Windows Server 2016+
- **macOS**: macOS 10.15+

## 🚀 快速开始

### 1. 环境准备

#### 安装 Node.js
```bash
# Ubuntu/Debian
curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash -
sudo apt-get install -y nodejs

# CentOS/RHEL
curl -fsSL https://rpm.nodesource.com/setup_18.x | sudo bash -
sudo yum install -y nodejs

# Windows (使用 Chocolatey)
choco install nodejs

# macOS (使用 Homebrew)
brew install node
```

#### 验证安装
```bash
node --version  # 应显示 v16.0.0 或更高版本
npm --version   # 应显示 npm 版本
```

### 2. 项目安装

#### 方式一：NPM 安装 (推荐)
```bash
# 在 Koishi 项目中安装
npm install koishi-plugin-mochi-link

# 或使用 yarn
yarn add koishi-plugin-mochi-link
```

#### 方式二：源码安装
```bash
# 克隆项目
git clone https://github.com/your-org/koishi-plugin-mochi-link.git
cd koishi-plugin-mochi-link

# 安装依赖
npm install

# 构建项目
npm run build
```

### 3. 基础配置

#### Koishi 配置文件 (koishi.yml)
```yaml
plugins:
  mochi-link:
    # WebSocket 服务配置
    websocket:
      port: 8080
      host: "0.0.0.0"
    
    # HTTP API 配置
    http:
      port: 8081
      host: "0.0.0.0"
      cors: true
    
    # 数据库配置
    database:
      prefix: "mochi_"
    
    # 安全配置
    security:
      tokenExpiry: 86400  # 24小时
      maxConnections: 100
    
    # 监控配置
    monitoring:
      reportInterval: 30
      historyRetention: 7

# 数据库配置 (必需)
database:
  # SQLite (开发环境)
  sqlite:
    path: "./data/database.db"
  
  # 或 MySQL (生产环境)
  # mysql:
  #   host: "localhost"
  #   port: 3306
  #   user: "koishi"
  #   password: "your_password"
  #   database: "koishi"
```

### 4. 启动服务

```bash
# 开发模式
npm run dev

# 生产模式
npm start

# 或使用 Koishi CLI
koishi start
```

## 📦 完整部署指南

### 部署环境说明

系统支持三种部署环境：

- **development**: 开发环境，用于本地开发和测试
- **staging**: 预发布环境，用于测试和验证
- **production**: 生产环境，用于正式部署

### 自动化部署

项目提供了完整的自动化部署脚本，支持环境检测、配置生成、构建和启动。

#### 1. 初始化部署环境

```bash
# 自动检测环境并初始化
npm run deploy:init

# 或指定特定环境
npm run deploy:init:dev      # 开发环境
npm run deploy:init:staging  # 预发布环境
npm run deploy:init:prod     # 生产环境
```

初始化过程会：
- 检查系统要求（Node.js 版本、依赖等）
- 生成环境特定的配置文件
- 创建启动脚本和 PM2 配置
- 验证环境变量

#### 2. 构建项目

```bash
# 构建并运行测试
npm run deploy:build
```

构建过程包括：
- 清理之前的构建文件
- TypeScript 编译
- 运行完整测试套件（678+ 测试）
- 生成生产就绪的代码

#### 3. 验证配置

```bash
# 验证当前环境配置
npm run deploy:validate

# 或验证特定环境
node scripts/deploy.js validate production
```

#### 4. 启动应用

```bash
# 启动应用
npm run deploy:start

# 或使用脚本
node scripts/deploy.js start production
```

#### 5. 管理应用

```bash
# 检查状态
npm run deploy:status

# 停止应用
npm run deploy:stop
```

### 手动部署步骤

#### 1. 服务器准备

```bash
# 更新系统
sudo apt update && sudo apt upgrade -y

# 安装必要软件
sudo apt install -y curl wget git build-essential

# 安装 Node.js 18
curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash -
sudo apt-get install -y nodejs

# 安装 PM2 (生产环境进程管理)
sudo npm install -g pm2

# 创建应用用户
sudo useradd -m -s /bin/bash mochi
sudo usermod -aG sudo mochi
```

#### 2. 项目部署

```bash
# 切换到应用用户
sudo su - mochi

# 克隆项目
git clone https://github.com/your-org/koishi-plugin-mochi-link.git
cd koishi-plugin-mochi-link

# 安装依赖
npm ci --production

# 构建项目
npm run build
```

#### 3. 配置文件设置

```bash
# 创建配置目录
mkdir -p config

# 生成生产环境配置
node scripts/deploy.js init production
```

编辑生成的配置文件 `config/deployment.json`：

```json
{
  "environment": "production",
  "version": "1.0.0",
  "services": {
    "websocket": {
      "enabled": true,
      "port": 8080,
      "host": "0.0.0.0",
      "ssl": {
        "enabled": true,
        "certPath": "/etc/ssl/certs/mochi-link.crt",
        "keyPath": "/etc/ssl/private/mochi-link.key"
      }
    },
    "http": {
      "enabled": true,
      "port": 8081,
      "host": "0.0.0.0",
      "cors": false
    }
  },
  "security": {
    "tokenExpiry": 86400,
    "maxConnections": 500,
    "rateLimiting": {
      "enabled": true,
      "windowMs": 60000,
      "maxRequests": 100
    },
    "encryption": {
      "enabled": true,
      "algorithm": "AES-256-GCM"
    }
  }
}
```

#### 4. 环境变量配置

创建 `.env` 文件：

```bash
# 环境配置
NODE_ENV=production
MOCHI_ENV=production

# 服务端口
MOCHI_WS_PORT=8080
MOCHI_HTTP_PORT=8081

# 数据库配置
MOCHI_DB_PREFIX=mochi_

# 安全配置
MOCHI_TOKEN_EXPIRY=86400
MOCHI_MAX_CONNECTIONS=500

# 日志配置
MOCHI_LOG_LEVEL=warn
```

#### 5. SSL 证书配置 (HTTPS/WSS)

```bash
# 使用 Let's Encrypt 获取免费证书
sudo apt install -y certbot

# 获取证书 (替换 your-domain.com)
sudo certbot certonly --standalone -d your-domain.com

# 复制证书到应用目录
sudo cp /etc/letsencrypt/live/your-domain.com/fullchain.pem /etc/ssl/certs/mochi-link.crt
sudo cp /etc/letsencrypt/live/your-domain.com/privkey.pem /etc/ssl/private/mochi-link.key

# 设置权限
sudo chown mochi:mochi /etc/ssl/certs/mochi-link.crt
sudo chown mochi:mochi /etc/ssl/private/mochi-link.key
sudo chmod 600 /etc/ssl/private/mochi-link.key
```

#### 6. 数据库配置

##### SQLite (简单部署)
```bash
# 创建数据目录
mkdir -p data
chmod 755 data
```

##### MySQL (推荐生产环境)
```bash
# 安装 MySQL
sudo apt install -y mysql-server

# 安全配置
sudo mysql_secure_installation

# 创建数据库和用户
sudo mysql -u root -p
```

```sql
CREATE DATABASE koishi CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'mochi'@'localhost' IDENTIFIED BY 'secure_password_here';
GRANT ALL PRIVILEGES ON koishi.* TO 'mochi'@'localhost';
FLUSH PRIVILEGES;
EXIT;
```

#### 7. 防火墙配置

```bash
# 启用 UFW 防火墙
sudo ufw enable

# 允许 SSH
sudo ufw allow ssh

# 允许应用端口
sudo ufw allow 8080/tcp  # WebSocket
sudo ufw allow 8081/tcp  # HTTP API

# 允许 HTTPS (如果使用反向代理)
sudo ufw allow 443/tcp
sudo ufw allow 80/tcp

# 检查状态
sudo ufw status
```

#### 8. 启动应用

```bash
# 使用 PM2 启动
pm2 start ecosystem.config.js --env production

# 设置开机自启
pm2 startup
pm2 save

# 检查状态
pm2 status
pm2 logs mochi-link
```

### Docker 部署

#### 1. Dockerfile

```dockerfile
FROM node:18-alpine

# 设置工作目录
WORKDIR /app

# 复制 package 文件
COPY package*.json ./

# 安装依赖
RUN npm ci --only=production

# 复制源代码
COPY . .

# 构建项目
RUN npm run build

# 创建非 root 用户
RUN addgroup -g 1001 -S nodejs
RUN adduser -S mochi -u 1001

# 设置权限
RUN chown -R mochi:nodejs /app
USER mochi

# 暴露端口
EXPOSE 8080 8081

# 健康检查
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD curl -f http://localhost:8081/health || exit 1

# 启动命令
CMD ["npm", "start"]
```

#### 2. docker-compose.yml

```yaml
version: '3.8'

services:
  mochi-link:
    build: .
    container_name: mochi-link
    restart: unless-stopped
    ports:
      - "8080:8080"  # WebSocket
      - "8081:8081"  # HTTP API
    environment:
      - NODE_ENV=production
      - MOCHI_ENV=production
      - MOCHI_WS_PORT=8080
      - MOCHI_HTTP_PORT=8081
      - MOCHI_DB_PREFIX=mochi_
      - MOCHI_LOG_LEVEL=info
    volumes:
      - ./data:/app/data
      - ./logs:/app/logs
      - ./config:/app/config
    depends_on:
      - mysql
    networks:
      - mochi-network

  mysql:
    image: mysql:8.0
    container_name: mochi-mysql
    restart: unless-stopped
    environment:
      - MYSQL_ROOT_PASSWORD=root_password_here
      - MYSQL_DATABASE=koishi
      - MYSQL_USER=mochi
      - MYSQL_PASSWORD=mochi_password_here
    volumes:
      - mysql_data:/var/lib/mysql
    networks:
      - mochi-network

  nginx:
    image: nginx:alpine
    container_name: mochi-nginx
    restart: unless-stopped
    ports:
      - "80:80"
      - "443:443"
    volumes:
      - ./nginx.conf:/etc/nginx/nginx.conf
      - ./ssl:/etc/ssl/certs
    depends_on:
      - mochi-link
    networks:
      - mochi-network

volumes:
  mysql_data:

networks:
  mochi-network:
    driver: bridge
```

#### 3. 启动 Docker 部署

```bash
# 构建并启动
docker-compose up -d

# 查看日志
docker-compose logs -f mochi-link

# 停止服务
docker-compose down
```

### 反向代理配置 (Nginx)

#### nginx.conf

```nginx
events {
    worker_connections 1024;
}

http {
    upstream mochi_http {
        server mochi-link:8081;
    }
    
    upstream mochi_ws {
        server mochi-link:8080;
    }

    # HTTP API 代理
    server {
        listen 80;
        server_name your-domain.com;
        
        # 重定向到 HTTPS
        return 301 https://$server_name$request_uri;
    }

    # HTTPS API 代理
    server {
        listen 443 ssl http2;
        server_name your-domain.com;

        ssl_certificate /etc/ssl/certs/mochi-link.crt;
        ssl_certificate_key /etc/ssl/private/mochi-link.key;
        
        # SSL 配置
        ssl_protocols TLSv1.2 TLSv1.3;
        ssl_ciphers ECDHE-RSA-AES256-GCM-SHA512:DHE-RSA-AES256-GCM-SHA512:ECDHE-RSA-AES256-GCM-SHA384:DHE-RSA-AES256-GCM-SHA384;
        ssl_prefer_server_ciphers off;

        # API 路由
        location /api/ {
            proxy_pass http://mochi_http;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
        }

        # WebSocket 代理
        location /ws {
            proxy_pass http://mochi_ws;
            proxy_http_version 1.1;
            proxy_set_header Upgrade $http_upgrade;
            proxy_set_header Connection "upgrade";
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;
            proxy_read_timeout 86400;
        }

        # 健康检查
        location /health {
            proxy_pass http://mochi_http;
            access_log off;
        }
    }
}
```

## 🔧 配置详解

### 环境变量配置

| 变量名 | 描述 | 默认值 | 示例 |
|--------|------|--------|------|
| `NODE_ENV` | Node.js 环境 | `development` | `production` |
| `MOCHI_ENV` | Mochi-Link 环境 | `development` | `production` |
| `MOCHI_WS_PORT` | WebSocket 端口 | `8080` | `8080` |
| `MOCHI_HTTP_PORT` | HTTP API 端口 | `8081` | `8081` |
| `MOCHI_DB_PREFIX` | 数据库表前缀 | `mochi_` | `mochi_prod_` |
| `MOCHI_TOKEN_EXPIRY` | 令牌过期时间(秒) | `86400` | `86400` |
| `MOCHI_MAX_CONNECTIONS` | 最大连接数 | `100` | `500` |
| `MOCHI_LOG_LEVEL` | 日志级别 | `info` | `warn` |

### 配置文件结构

#### deployment.json 配置说明

```json
{
  "environment": "production",           // 环境类型
  "version": "1.0.0",                   // 版本号
  "buildTime": "2024-01-01T00:00:00Z",  // 构建时间
  
  "services": {
    "websocket": {
      "enabled": true,                   // 启用 WebSocket 服务
      "port": 8080,                     // WebSocket 端口
      "host": "0.0.0.0",               // 绑定地址
      "ssl": {
        "enabled": true,                // 启用 SSL/TLS
        "certPath": "/path/to/cert",    // 证书路径
        "keyPath": "/path/to/key"       // 私钥路径
      }
    },
    "http": {
      "enabled": true,                  // 启用 HTTP API
      "port": 8081,                    // HTTP 端口
      "host": "0.0.0.0",              // 绑定地址
      "cors": false                    // CORS 支持
    },
    "database": {
      "prefix": "mochi_",              // 表前缀
      "connectionTimeout": 30000,      // 连接超时
      "queryTimeout": 15000            // 查询超时
    }
  },
  
  "security": {
    "tokenExpiry": 86400,              // 令牌过期时间
    "maxConnections": 500,             // 最大连接数
    "rateLimiting": {
      "enabled": true,                 // 启用速率限制
      "windowMs": 60000,              // 时间窗口
      "maxRequests": 100              // 最大请求数
    },
    "encryption": {
      "enabled": true,                 // 启用加密
      "algorithm": "AES-256-GCM"      // 加密算法
    }
  },
  
  "monitoring": {
    "enabled": true,                   // 启用监控
    "reportInterval": 30,              // 报告间隔(秒)
    "historyRetention": 90,            // 历史保留(天)
    "healthCheck": {
      "interval": 30000,               // 健康检查间隔
      "timeout": 5000                  // 检查超时
    },
    "metrics": {
      "enabled": true,                 // 启用指标收集
      "interval": 60000,               // 收集间隔
      "retention": 90                  // 保留天数
    }
  },
  
  "logging": {
    "level": "warn",                   // 日志级别
    "auditRetention": 365,             // 审计日志保留(天)
    "fileLogging": {
      "enabled": true,                 // 启用文件日志
      "path": "/var/log/mochi-link",   // 日志路径
      "maxSize": "500MB",              // 最大文件大小
      "maxFiles": 20                   // 最大文件数
    }
  },
  
  "performance": {
    "caching": {
      "enabled": true,                 // 启用缓存
      "maxSize": 1000,                // 最大缓存条目
      "ttl": 1800                     // 生存时间(秒)
    },
    "connectionPool": {
      "enabled": true,                 // 启用连接池
      "minConnections": 5,             // 最小连接数
      "maxConnections": 50,            // 最大连接数
      "idleTimeout": 300000            // 空闲超时
    },
    "optimization": {
      "enabled": true,                 // 启用优化
      "queryCache": true,              // 查询缓存
      "compression": true              // 启用压缩
    }
  }
}
```

## 🔍 监控与维护

### 健康检查

系统提供多层次的健康检查：

#### 1. HTTP 健康检查端点

```bash
# 基础健康检查
curl http://localhost:8081/health

# 详细健康信息
curl http://localhost:8081/health/detailed

# 系统指标
curl http://localhost:8081/metrics
```

#### 2. 系统监控指标

- **系统资源**：CPU、内存、磁盘使用率
- **服务状态**：WebSocket、HTTP、数据库连接状态
- **性能指标**：响应时间、吞吐量、错误率
- **业务指标**：连接服务器数、活跃用户数、命令执行次数

#### 3. 日志监控

```bash
# 查看实时日志
pm2 logs mochi-link

# 查看错误日志
pm2 logs mochi-link --err

# 查看特定时间段日志
pm2 logs mochi-link --lines 100
```

### 性能优化

#### 1. 数据库优化

```sql
-- 创建索引优化查询性能
CREATE INDEX idx_servers_owner ON minecraft_servers(owner_id);
CREATE INDEX idx_audit_timestamp ON audit_logs(timestamp);
CREATE INDEX idx_tokens_server ON api_tokens(server_id);
```

#### 2. 缓存配置

```json
{
  "performance": {
    "caching": {
      "enabled": true,
      "maxSize": 1000,        // 根据内存调整
      "ttl": 1800            // 30分钟缓存
    }
  }
}
```

#### 3. 连接池优化

```json
{
  "performance": {
    "connectionPool": {
      "enabled": true,
      "minConnections": 5,    // 最小连接数
      "maxConnections": 50,   // 根据负载调整
      "idleTimeout": 300000   // 5分钟空闲超时
    }
  }
}
```

### 备份策略

#### 1. 数据库备份

```bash
# MySQL 备份脚本
#!/bin/bash
DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_DIR="/backup/mysql"
DB_NAME="koishi"

mkdir -p $BACKUP_DIR

# 创建备份
mysqldump -u mochi -p$MYSQL_PASSWORD $DB_NAME > $BACKUP_DIR/mochi_backup_$DATE.sql

# 压缩备份
gzip $BACKUP_DIR/mochi_backup_$DATE.sql

# 删除7天前的备份
find $BACKUP_DIR -name "*.sql.gz" -mtime +7 -delete
```

#### 2. 配置文件备份

```bash
# 配置备份脚本
#!/bin/bash
DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_DIR="/backup/config"
APP_DIR="/home/mochi/koishi-plugin-mochi-link"

mkdir -p $BACKUP_DIR

# 备份配置文件
tar -czf $BACKUP_DIR/config_backup_$DATE.tar.gz \
  $APP_DIR/config/ \
  $APP_DIR/ecosystem.config.js \
  $APP_DIR/.env

# 删除30天前的备份
find $BACKUP_DIR -name "*.tar.gz" -mtime +30 -delete
```

#### 3. 自动备份设置

```bash
# 添加到 crontab
crontab -e

# 每天凌晨2点备份数据库
0 2 * * * /home/mochi/scripts/backup_database.sh

# 每周日凌晨3点备份配置
0 3 * * 0 /home/mochi/scripts/backup_config.sh
```

### 故障排除

#### 常见问题及解决方案

##### 1. 服务无法启动

```bash
# 检查端口占用
netstat -tlnp | grep :8080
netstat -tlnp | grep :8081

# 检查日志
pm2 logs mochi-link --lines 50

# 检查配置文件
node scripts/deploy.js validate
```

##### 2. 数据库连接失败

```bash
# 检查数据库服务状态
sudo systemctl status mysql

# 测试数据库连接
mysql -u mochi -p -h localhost koishi

# 检查数据库配置
cat config/deployment.json | grep -A 10 "database"
```

##### 3. WebSocket 连接问题

```bash
# 检查防火墙设置
sudo ufw status

# 测试 WebSocket 连接
wscat -c ws://localhost:8080

# 检查 SSL 证书 (如果启用)
openssl x509 -in /etc/ssl/certs/mochi-link.crt -text -noout
```

##### 4. 性能问题

```bash
# 检查系统资源
htop
df -h
free -h

# 检查应用性能
pm2 monit

# 分析慢查询 (MySQL)
mysql -u root -p -e "SHOW PROCESSLIST;"
```

### 安全维护

#### 1. 定期安全更新

```bash
# 更新系统包
sudo apt update && sudo apt upgrade -y

# 更新 Node.js 依赖
npm audit
npm audit fix

# 更新 PM2
sudo npm update -g pm2
```

#### 2. 证书更新

```bash
# 更新 Let's Encrypt 证书
sudo certbot renew

# 重启服务以加载新证书
pm2 restart mochi-link
```

#### 3. 安全审计

```bash
# 检查开放端口
nmap localhost

# 检查登录日志
sudo tail -f /var/log/auth.log

# 检查应用访问日志
tail -f /var/log/mochi-link/access.log
```

## 📚 API 文档

Mochi-Link 提供了完整的 API 接口文档，包括 HTTP REST API 和 WebSocket 实时通信接口。详细的 API 文档请参考：

### [完整 API 接口文档](API接口文档.md)

该文档包含以下主要内容：

#### 1. U-WBP v2 协议规范
- 消息格式和类型定义
- 握手流程和心跳机制
- 协议版本兼容性

#### 2. HTTP REST API
- 服务器管理 API（注册、查询、更新、删除）
- 玩家管理 API（列表、详情、踢出、私聊）
- 白名单管理 API（查询、添加、移除）
- 命令执行 API（单命令、批量执行）
- 服务器状态和性能监控 API
- 权限管理和审计日志 API

#### 3. WebSocket 实时接口
- 连接建立和认证
- 核心操作接口（状态查询、玩家管理、命令执行）
- 事件订阅和管理
- 实时数据推送

#### 4. 数据模型
- 服务器配置和连接配置
- 玩家信息和身份识别
- 服务器状态和性能指标

#### 5. 错误处理
- 错误响应格式
- 常见错误码说明
- WebSocket 错误消息格式

#### 6. 认证与权限
- API Token 认证
- WebSocket 认证
- 权限模型和角色定义
- 权限检查流程

#### 7. 事件系统
- 事件类型分类（玩家、服务器、性能、系统）
- 事件格式和订阅管理
- 实时事件推送机制

### 快速开始示例

#### HTTP API 基础使用
```http
# 获取服务器列表
GET /api/servers
Authorization: Bearer <your-token>

# 执行命令
POST /api/servers/{serverId}/commands
Content-Type: application/json
Authorization: Bearer <your-token>

{
  "command": "say Hello from API",
  "timeout": 5000
}
```

#### WebSocket 基础使用
```javascript
// 连接到 WebSocket 服务器
const ws = new WebSocket('ws://localhost:8080/ws');

// 发送认证消息
ws.send(JSON.stringify({
  type: 'request',
  id: 'auth_001',
  op: 'system.auth',
  data: {
    serverId: 'your-server-id',
    token: 'your-api-token'
  }
}));

// 订阅玩家加入事件
ws.send(JSON.stringify({
  type: 'request',
  id: 'sub_001',
  op: 'event.subscribe',
  data: {
    serverId: 'your-server-id',
    eventTypes: ['player.join']
  }
}));
```

### 重要提示
- 所有 API 请求都需要有效的认证令牌
- WebSocket 连接支持正向和反向两种模式
- 详细的参数说明和示例请参考完整 API 文档

## 🧪 测试

### 运行测试

```bash
# 运行所有测试
npm test

# 运行单元测试
npm run test:unit

# 运行集成测试
npm run test:integration

# 运行属性测试
npm run test:property

# 生成测试覆盖率报告
npm run test:coverage

# 监听模式运行测试
npm run test:watch
```

### 测试统计

- **总测试数**: 725+
- **通过率**: 93.5%
- **测试类型**:
  - 单元测试: 208 个
  - 属性测试: 4 个属性
  - 集成测试: 12 个
  - 端到端测试: 5 个套件

### 测试覆盖率

- **行覆盖率**: >95%
- **函数覆盖率**: 100%
- **分支覆盖率**: >90%
- **语句覆盖率**: >95%

## 🤝 贡献指南

### 开发环境设置

```bash
# 克隆项目
git clone https://github.com/your-org/koishi-plugin-mochi-link.git
cd koishi-plugin-mochi-link

# 安装依赖
npm install

# 启动开发模式
npm run dev

# 运行测试
npm test
```

### 代码规范

- **TypeScript**: 使用严格模式
- **ESLint**: 遵循项目 ESLint 配置
- **Prettier**: 代码格式化
- **测试**: 新功能必须包含测试
- **文档**: 更新相关文档

### 提交流程

1. Fork 项目
2. 创建功能分支: `git checkout -b feature/amazing-feature`
3. 提交更改: `git commit -m 'Add amazing feature'`
4. 推送分支: `git push origin feature/amazing-feature`
5. 创建 Pull Request

## 📄 许可证

本项目采用 MIT 许可证 - 查看 [LICENSE](LICENSE) 文件了解详情。

## 🆘 支持与帮助

### 获取帮助

- 📧 **邮件支持**: support@example.com
- 💬 **QQ群**: 123456789
- 🐛 **问题反馈**: [GitHub Issues](https://github.com/your-org/koishi-plugin-mochi-link/issues)
- 📖 **详细文档**: [项目 Wiki](https://github.com/your-org/koishi-plugin-mochi-link/wiki)

### 常见问题

#### Q: 支持哪些 Minecraft 版本？
A: 支持所有主流 Minecraft 版本，通过 Connector Bridge 适配不同核心。

#### Q: 可以同时管理多少台服务器？
A: 理论上无限制，实际取决于服务器性能。推荐单实例管理 50-100 台服务器。

#### Q: 是否支持集群部署？
A: 当前版本支持单实例部署，集群部署功能在开发计划中。

#### Q: 如何备份数据？
A: 参考部署指南中的备份策略章节，提供了完整的备份方案。

---

**Mochi-Link (大福连)** - 让 Minecraft 服务器管理变得简单而统一 🎮✨