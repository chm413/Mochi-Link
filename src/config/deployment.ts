...
const DEFAULT_CONFIGS: Record<DeploymentEnvironment, DeploymentConfig> = {
  development: {
...
export class DeploymentConfigManager {
  ...
  private loadConfigFromFile(): Partial<DeploymentConfig> {
  ...
  private mergeConfigs(
    ...
    fileConfig: Partial<DeploymentConfig>
  ): DeploymentConfig {
    return this.deepMerge(defaultConfig, fileConfig) as DeploymentConfig;
  }
...
export class EnvironmentDetector {
  ...
  static detectEnvironment(): DeploymentEnvironment {
    ...
    if (mochiEnv === 'production') return 'production';
    if (mochiEnv === 'staging') return 'staging';
    if (mochiEnv === 'development') return 'development';
  ...
  static isDocker(): boolean {
    return !!process.env.DOCKER_CONTAINER || existsSync('/.dockerenv');
  }
...
export class ConfigurationUtils {
  ...
  static applyEnvironmentOverrides(config: DeploymentConfig): DeploymentConfig {
    ...
    if (process.env.MOCHI_LOG_LEVEL) {
      overrideConfig.logging.level = process.env.MOCHI_LOG_LEVEL as any;
    }
   ...
   */
  static validateEnvironmentVariables(): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
