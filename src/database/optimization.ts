...
class QueryCache {
  ...
  getStats(): { hitRate: number; size: number; hits: number; misses: number } {
...
export class DatabaseQueryOptimizer {
   ...
   */
  async getServersBatch(serverIds: string[]): Promise<any[]> {
  ...
  async getUserPermissionsOptimized(userId: string): Promise<any[]> {
    ...
    const acls = await this.ctx.database.get(TableNames.serverAcl as any, {
      user_id: userId,
      $or: [
        { expires_at: null },
  ...
  async getAuditLogsOptimized(filters: {
    ...
    const [logs, totalCount] = await Promise.all([
      this.ctx.database.get(TableNames.auditLogs as any, query, {
        limit: filters.limit || 100,
        offset: filters.offset || 0
      }),
      this.ctx.database.get(TableNames.auditLogs as any, query, ['id']).then(results => results.length)
    ]);
   ...
   */
  async optimizePendingOperations(serverId: string): Promise<number> {
    const startTime = Date.now();
    
    // Get all pending operations for the server
    const operations = await this.ctx.database.get(TableNames.pendingOperations as any, {
      server_id: serverId,
      status: 'pending'
    });

    let optimizedCount = 0;
    const operationsByTarget = new Map<string, any[]>();
  ...
  async warmUpCache(): Promise<void> {
    ...
    try {
      ...
      const recentAcls = await this.ctx.database.get(TableNames.serverAcl as any, {
        $or: [
          { expires_at: null },
          { expires_at: { $gt: new Date() } }
        ]
      ...
      const userAcls = new Map<string, any[]>();
  ...
  getPerformanceStats(): {
    ...
    slowQueries: number;
    cacheStats: any;
    queryTypeStats: Record<string, { count: number; avgTime: number }>;
  } {
    const cacheStats = this.queryCache.getStats();
    const queryTypeStats: Record<string, { count: number; avgTime: number }> = {};
  ...
  async generateIndexRecommendations(): Promise<IndexRecommendation[]> {
    ...
    const slowQueries = this.performanceMetrics.filter(m => 
   ...
   */
  private areContradictoryOperations(op1: any, op2: any): boolean {
    if (op1.target !== op2.target) return false;
