...
import { RequestContext, ValidationResult, ValidationError } from '../types';
...
interface ValidationSchema {
  ...
  pattern?: string;
  enum?: any[];
  format?: 'email' | 'uuid' | 'date-time' | 'hostname' | 'ipv4' | 'ipv6';
}
...
export class ValidationMiddleware implements HTTPMiddleware {
  ...
  private validateBasicRequest(request: HTTPRequest): ValidationResult {
  ...
  private validateRequestSchema(request: HTTPRequest): ValidationResult {
  ...
  private validateValue(value: any, schema: ValidationSchema, fieldPath: string): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  private extractPathParams(schemaPath: string, actualPath: string): Record<string, string> {
  ...
  private sanitizeRequest(request: HTTPRequest): { body: any; query: any } {
    return {
  ...
  private sendValidationError(response: ServerResponse, errors: ValidationError[]): void {
