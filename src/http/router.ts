...
import { HTTPRequest, APIResponse, PaginatedResponse } from './types';
...
export class APIRouter {
  ...
  public async route(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  private getHandler(method: string, path: string): RouteHandler | null {
    // System endpoints
    if (method === 'GET' && path === '/api/health') return this.getSystemHealth.bind(this);
    if (method === 'GET' && path === '/api/stats') return this.getSystemStats.bind(this);

    // Server management endpoints
    if (method === 'GET' && path === '/api/servers') return this.getServers.bind(this);
    if (method === 'POST' && path === '/api/servers') return this.createServer.bind(this);
    if (method === 'GET' && this.matchPath('/api/servers/:serverId', path)) return this.getServer.bind(this);
    if (method === 'PUT' && this.matchPath('/api/servers/:serverId', path)) return this.updateServer.bind(this);
    if (method === 'DELETE' && this.matchPath('/api/servers/:serverId', path)) return this.deleteServer.bind(this);

    // Player management endpoints
    if (method === 'GET' && this.matchPath('/api/servers/:serverId/players', path)) return this.getPlayers.bind(this);
    if (method === 'GET' && this.matchPath('/api/servers/:serverId/players/:playerId', path)) return this.getPlayer.bind(this);
    if (method === 'POST' && this.matchPath('/api/servers/:serverId/players/:playerId/kick', path)) return this.kickPlayer.bind(this);

    // Whitelist management endpoints
    if (method === 'GET' && this.matchPath('/api/servers/:serverId/whitelist', path)) return this.getWhitelist.bind(this);
    if (method === 'POST' && this.matchPath('/api/servers/:serverId/whitelist', path)) return this.addToWhitelist.bind(this);
    if (method === 'DELETE' && this.matchPath('/api/servers/:serverId/whitelist/:playerId', path)) return this.removeFromWhitelist.bind(this);

    // Ban management endpoints
    if (method === 'GET' && this.matchPath('/api/servers/:serverId/bans', path)) return this.getBans.bind(this);
    if (method === 'POST' && this.matchPath('/api/servers/:serverId/bans', path)) return this.createBan.bind(this);
    if (method === 'PUT' && this.matchPath('/api/servers/:serverId/bans/:banId', path)) return this.updateBan.bind(this);
    if (method === 'DELETE' && this.matchPath('/api/servers/:serverId/bans/:banId', path)) return this.deleteBan.bind(this);

    // Command execution endpoints
    if (method === 'POST' && this.matchPath('/api/servers/:serverId/commands', path)) return this.executeCommand.bind(this);
    if (method === 'POST' && this.matchPath('/api/servers/:serverId/actions', path)) return this.executeQuickAction.bind(this);

    // Monitoring endpoints
    if (method === 'GET' && this.matchPath('/api/servers/:serverId/status', path)) return this.getServerStatus.bind(this);
    if (method === 'GET' && this.matchPath('/api/servers/:serverId/performance', path)) return this.getPerformanceHistory.bind(this);
    if (method === 'GET' && this.matchPath('/api/servers/:serverId/alerts', path)) return this.getAlerts.bind(this);
    if (method === 'POST' && this.matchPath('/api/servers/:serverId/alerts/:alertId/acknowledge', path)) return this.acknowledgeAlert.bind(this);

    // Real-time monitoring endpoints
    if (method === 'GET' && this.matchPath('/api/servers/:serverId/metrics/current', path)) return this.getCurrentMetrics.bind(this);
    if (method === 'GET' && this.matchPath('/api/servers/:serverId/metrics/summary', path)) return this.getMetricsSummary.bind(this);

    // Batch operations
    if (method === 'POST' && path === '/api/batch/commands') return this.executeBatchCommands.bind(this);
    if (method === 'POST' && path === '/api/batch/actions') return this.executeBatchActions.bind(this);

    // Audit log endpoints
    if (method === 'GET' && path === '/api/audit') return this.getAuditLogs.bind(this);

    // Authentication endpoints
    if (method === 'POST' && path === '/api/auth/verify') return this.verifyToken.bind(this);
    if (method === 'POST' && path === '/api/auth/tokens') return this.createToken.bind(this);

    // Binding management endpoints
    if (method === 'GET' && path === '/api/bindings') return this.getBindings.bind(this);
    if (method === 'POST' && path === '/api/bindings') return this.createBinding.bind(this);
    if (method === 'GET' && this.matchPath('/api/bindings/:bindingId', path)) return this.getBinding.bind(this);
    if (method === 'PUT' && this.matchPath('/api/bindings/:bindingId', path)) return this.updateBinding.bind(this);
    if (method === 'DELETE' && this.matchPath('/api/bindings/:bindingId', path)) return this.deleteBinding.bind(this);
    if (method === 'GET' && path === '/api/bindings/stats') return this.getBindingStats.bind(this);
    if (method === 'POST' && path === '/api/bindings/batch') return this.createBindingsBatch.bind(this);
    if (method === 'GET' && this.matchPath('/api/groups/:groupId/routes', path)) return this.getGroupRoutes.bind(this);
  ...
  async getSystemHealth(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async getSystemStats(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async getServers(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async createServer(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async getServer(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async updateServer(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async deleteServer(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async getPlayers(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async getPlayer(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
  ...
  async kickPlayer(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      const playerId = req.context.playerId;
      ...
      const result = await this.serviceManager.playerManager.kickPlayer(
        serverId, 
        playerId, 
        req.body.reason, 
        req.context.userId
  ...
  async getWhitelist(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async addToWhitelist(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
      ...
      const result = await this.serviceManager.whitelistManager.addToWhitelist(
        serverId,
        req.body.playerId,
        req.body.playerName,
        req.context.userId
  ...
  async removeFromWhitelist(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      const playerId = req.context.playerId;
      ...
      const result = await this.serviceManager.whitelistManager.removeFromWhitelist(
        serverId,
        playerId,
        req.context.userId
      );
  ...
  async getBans(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async createBan(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async updateBan(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
  ...
  async deleteBan(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      const banId = req.context.banId;
  ...
  async executeCommand(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
      ...
      // Check permissions
      const permissionCheck = await this.serviceManager.permissionManager.checkPermission(
        req.context.userId,
        serverId,
        'commands.execute'
      );
      ...
      const result = await this.serviceManager.commandService.executeCommand(
        serverId,
        req.body.command,
        req.context.userId,
        {
  ...
  async executeQuickAction(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
      ...
      const result = await this.serviceManager.commandService.executeQuickAction(
        serverId,
        req.body,
        req.context.userId
      );
  ...
  async getServerStatus(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async getPerformanceHistory(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
      ...
      const startTime = req.query?.startTime ? new Date(req.query.startTime as string) : new Date(Date.now() - 3600000);
      const endTime = req.query?.endTime ? new Date(req.query.endTime as string) : new Date();
      const interval = parseInt((req.query?.interval as string) || '5') || 5;

      const result = await this.serviceManager.monitoringService.getHistoricalData(
        serverId,
  ...
  async getAlerts(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async acknowledgeAlert(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async getCurrentMetrics(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async getMetricsSummary(req: HTTPRequest): Promise<APIResponse> {
    try {
      const serverId = req.context.serverId;
      if (!serverId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Server ID is required', req.context.requestId);
  ...
  async executeBatchCommands(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async executeBatchActions(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async getAuditLogs(req: HTTPRequest): Promise<APIResponse> {
    try {
      const page = parseInt((req.query?.page as string) || '1') || 1;
      const limit = parseInt((req.query?.limit as string) || '20') || 20;
  ...
  async verifyToken(req: HTTPRequest): Promise<APIResponse> {
    try {
      if (!req.body.token) {
  ...
  async createToken(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async getBindings(req: HTTPRequest): Promise<APIResponse> {
    try {
      const page = parseInt((req.query?.page as string) || '1') || 1;
      const limit = parseInt((req.query?.limit as string) || '20') || 20;
      ...
      const filters = {
        ...
        serverId: req.query?.serverId as string,
        bindingType: req.query?.bindingType as any,
  ...
  async createBinding(req: HTTPRequest): Promise<APIResponse> {
    try {
      ...
      // Check permissions
      const permissionCheck = await this.serviceManager.permission.checkPermission(
        req.context.userId,
        req.body.serverId,
        'binding.create'
      );
  ...
  async getBinding(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async updateBinding(req: HTTPRequest): Promise<APIResponse> {
    try {
      const bindingId = parseInt(req.context.bindingId || '0');
      if (!bindingId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Binding ID is required', req.context.requestId);
  ...
  async deleteBinding(req: HTTPRequest): Promise<APIResponse> {
    try {
      const bindingId = parseInt(req.context.bindingId || '0');
      if (!bindingId) {
        return this.createErrorResponse('INVALID_REQUEST', 'Binding ID is required', req.context.requestId);
  ...
  async getBindingStats(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async createBindingsBatch(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  async getGroupRoutes(req: HTTPRequest): Promise<APIResponse> {
    try {
  ...
  private createSuccessResponse(data: any, requestId?: string, metadata?: any): APIResponse {
  ...
  private createPaginatedResponse(data: any[], pagination: any, requestId?: string, summary?: any): PaginatedResponse<any> {
  ...
  private createErrorResponse(error: string, message: string, requestId?: string): APIResponse {
    return {
      success: false,
      error,
      message,
