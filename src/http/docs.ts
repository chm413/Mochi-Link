...
import { generateOpenApiSpec, generateOpenApiYaml } from './openapi';
...
export class DocumentationServer {
   ...
   */
  async handleDocumentationRequest(request: HTTPRequest): Promise<APIResponse | null> {
   ...
   */
  private serveOpenApiJson(request: HTTPRequest): APIResponse {
    try {
   ...
   */
  private serveOpenApiYaml(request: HTTPRequest): APIResponse {
    try {
   ...
   */
  private serveVersionInfo(request: HTTPRequest): APIResponse {
   ...
   */
  private serveSwaggerUI(request: HTTPRequest): APIResponse {
   ...
   */
  private serveHealthCheck(request: HTTPRequest): APIResponse {
    return {
      success: true,
      data: {
        status: 'healthy',
   ...
   */
  private serveEndpointsList(request: HTTPRequest): APIResponse {
    const endpoints = [
      // System endpoints
...
<body>
        ...
        window.onload = function() {
            const ui = SwaggerUIBundle({
                ...
                responseInterceptor: function(response) {
                    // Response interceptor for debugging (client-side only)
                    return response;
                },
                onComplete: function() {
                    // Swagger UI loaded successfully (client-side only)
                },
                onFailure: function(error) {
                    // Failed to load Swagger UI (client-side only)
                    alert('Failed to load Swagger UI. Please check console for details.');
                }
            });
   ...
   */
  sendRawResponse(response: ServerResponse, content: string, contentType: string, statusCode = 200): void {
...
export class DocumentationMiddleware {
  constructor(private docServer: DocumentationServer) {}
      ...
      try {
        ...
        if (docResponse) {
          ...
          if (docResponse.headers?.['Content-Type'] === 'text/html') {
            this.docServer.sendRawResponse(
              response, 
              docResponse.data as string, 
              'text/html'
            );
