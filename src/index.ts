 ...
 */
function parseRegisterMessage(message: string): {
  id: string;
  name: string;
  host: string;
  port: number;
  ...
  try {
    ...
    // 提取其他参数
    const nameMatch = content.match(/--name\s+([^\s-]+(?:\s+[^\s-]+)*?)(?:\s+--|$)/);
    ...
    const type = typeMatch ? typeMatch[1].toLowerCase() as 'java' | 'bedrock' : 
                 (core.includes('nukkit') || core.includes('pmmp') || core.includes('bds') || core.includes('llbds')) ? 'bedrock' : 'java';
...
export function apply(ctx: Context, config: PluginConfig) {
    ...
    ctx.middleware(async (session, next) => {
        ...
        if (content && /^\/mochi\s+register\s+/i.test(content)) {
            ...
            if (!isInitialized || !dbManager) {
                await session.send('插件尚未初始化完成，请稍后再试');
                return;
            }
            ...
            try {
                ...
                if (!parsed) {
                    await session.send(
                      ...
                      '/mochi register creative --name 创造服 --host 192.168.1.100 --port 25566 --core fabric --type java'
                    );
                    return;
                }
                ...
                if (existing) {
                    await session.send(`❌ 服务器 ${parsed.id} 已存在`);
                    return;
                }
                ...
                logger.info(`Server registered: ${parsed.id} by user ${session.userId}`);
                return;
              } catch (error) {
                logger.error('Failed to register server:', error);
                await session.send('❌ 注册服务器失败: ' + (error as Error).message);
                return;
              }
        }
    ...
    // Register server - Level 3 (管理员) - 用于服务器自动注册
    ctx.command('mochi.server.register <message:text>', '注册服务器（自动识别）')
      .userFields(['authority'])
      .before(({ session }) => {
        if ((session?.user?.authority ?? 0) < 3) {
          return '权限不足：需要管理员权限（等级 3）';
      ...
      .action(async ({ session }, message) => {
        ...
        try {
          // 解析注册信息
          // 格式: /mochi register java-server --host 127.0.0.1 --port 25565 --core paper
          const parsed = parseRegisterMessage(message);
          
          if (!parsed) {
            return '注册信息格式错误\n' +
                   '正确格式: /mochi register <服务器ID> --name <名称> --host <地址> --port <端口> --core <核心类型>';
          }
          
          // Check if server already exists
          const existing = await dbManager.getServer(parsed.id);
          if (existing) {
            return `服务器 ${parsed.id} 已存在`;
          }
          ...
          return `✅ 服务器注册成功！\n\n` +
                 `服务器 ID: ${parsed.id}\n` +
                 `服务器名称: ${parsed.name}\n` +
                 `核心类型: ${parsed.type}/${parsed.core}\n` +
                 `连接地址: ${parsed.host}:${parsed.port}\n\n` +
                 `请在服务器上安装对应的连接器插件并配置连接信息。`;
        } catch (error) {
          logger.error('Failed to register server:', error);
          return '注册服务器失败: ' + (error as Error).message;
        }
      });
