...
export class ServerManager {
  ...
  async registerServer(
    ...
    try {
      ...
      const serverConfig: Omit<ServerConfig, 'createdAt' | 'updatedAt'> = {
        id: serverId,
        name: options.name,
      ...
      if (existing) {
        throw new MochiLinkError(
          `Server with ID ${serverId} already exists`,
          'SERVER_EXISTS',
          { serverId }
        );
      }
      ...
      await this.audit.logger.logError(
        'server.register',
        { serverName: options.name },
   ...
   */
  async getServerSummaries(userId?: string): Promise<ServerSummary[]> {
  ...
  async updateServer(
    ...
    try {
      ...
      if (!hasPermission.granted) {
        throw new MochiLinkError(
          `User ${operatorId} lacks permission to update server ${serverId}`,
          'PERMISSION_DENIED',
          { userId: operatorId, serverId, operation: 'server.update' }
        );
      }
  ...
  async deleteServer(
    ...
    try {
      ...
      if (!hasPermission.granted) {
        throw new MochiLinkError(
          `User ${operatorId} lacks permission to delete server ${serverId}`,
          'PERMISSION_DENIED',
          { userId: operatorId, serverId, operation: 'server.delete' }
        );
      }
      ...
      if (deleted) {
        ...
        await this.audit.logger.logSuccess(
          'server.delete',
          { serverId, serverName: existing.name },
          { userId: operatorId, ipAddress }
        );
  ...
  async updateServerStatus(
    ...
    try {
      // Update database
      await this.db.servers.updateServerStatus(
        serverId,
        status,
        status === 'online' ? new Date() : undefined
      );

      // Update cache
      const currentInfo = this.statusCache.get(serverId);
      const updatedInfo: ServerStatusInfo = {
        serverId,
        status,
   ...
   */
  async executeOnMultipleServers<T>(
    serverIds: string[],
    ...
    await Promise.all(
      semaphore.map(() => executeWithSemaphore())
    );
  ...
  async broadcastCommand(
    ...
    if (unauthorizedServers.length > 0) {
      throw new MochiLinkError(
        `User ${operatorId} lacks permission to execute commands on servers: ${unauthorizedServers.join(', ')}`,
        'PERMISSION_DENIED',
        { userId: operatorId, serverIds: unauthorizedServers, operation: 'server.command' }
      );
    }

    // Execute command on all authorized servers
    return await this.executeOnMultipleServers(
      serverIds,
      async (serverId) => {
  ...
  async switchConnectionMode(
    ...
    try {
      ...
      if (operatorId) {
        ...
        if (!hasPermission.granted) {
          throw new MochiLinkError(
            `User ${operatorId} lacks permission to switch connection mode for server ${serverId}`,
            'PERMISSION_DENIED',
            { userId: operatorId, serverId, operation: 'server.connection.switch' }
          );
        }
      }
      ...
      const updates: any = { connectionMode: newMode };
      if (newConfig) {
      ...
      if (operatorId) {
        await this.audit.logger.logSuccess(
          'server.connection.switch',
          { serverId, oldMode: existing.connectionMode, newMode },
          { userId: operatorId, ipAddress }
        );
      }
   ...
   */
  private scheduleConnection(serverId: string, delay: number): void {
    this.clearReconnectTimer(serverId);
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        this.ctx.logger('mochi-link:server').error(
          `Scheduled connection to ${serverId} failed:`, error
        );
      }
    }, delay);
   ...
   * Generate unique server ID
   */
  private generateServerId(name: string, coreType: CoreType): string {
   ...
   * Validate connection configuration
   */
  private validateConnectionConfig(mode: ConnectionMode, config: ConnectionConfig): void {
    switch (mode) {
      case 'plugin':
        if (!config.plugin) {
          throw new MochiLinkError(
            'Plugin connection configuration is required',
            'INVALID_CONFIG',
            { mode, config }
          );
        }
        if (!config.plugin.host || !config.plugin.port) {
          throw new MochiLinkError(
            'Plugin connection requires host and port',
            'INVALID_CONFIG',
            { mode, config }
          );
        }
      ...
      case 'rcon':
        if (!config.rcon) {
          throw new MochiLinkError(
            'RCON connection configuration is required',
            'INVALID_CONFIG',
            { mode, config }
          );
        }
        if (!config.rcon.host || !config.rcon.port || !config.rcon.password) {
          throw new MochiLinkError(
            'RCON connection requires host, port, and password',
            'INVALID_CONFIG',
            { mode, config }
          );
        }
      ...
      case 'terminal':
        if (!config.terminal) {
          throw new MochiLinkError(
            'Terminal connection configuration is required',
            'INVALID_CONFIG',
            { mode, config }
          );
        }
        if (!config.terminal.processId || !config.terminal.workingDir) {
          throw new MochiLinkError(
            'Terminal connection requires processId and workingDir',
            'INVALID_CONFIG',
            { mode, config }
          );
        }
   ...
   */
  private extractIPFromConnectionConfig(config: ConnectionConfig): string[] | undefined {
    const ips: string[] = [];
   ...
   */
  async cleanupPluginIntegration(serverId: string): Promise<void> {
    if (!this.pluginIntegration) {
      return;
    }
   ...
   */
  private createBridgeWrapper(serverId: string, connection: Connection): BaseConnectorBridge {
    const server = this.statusCache.get(serverId);
    if (!server) {
      throw new MochiLinkError(
        `Server ${serverId} not found in status cache`,
        'SERVER_NOT_FOUND',
        { serverId }
      );
    }
    ...
    return {
      ...
      async executeCommand(command: string, timeout?: number): Promise<CommandResult> {
        if (connection.status !== 'connected') {
          throw new ServerUnavailableError(`Server ${serverId} is not connected`, serverId);
        }
   ...
   * Create a Connection wrapper for backward compatibility
   */
  private createConnectionWrapper(adapter: ConnectionAdapter): Connection {
    return {
