...
export interface ConnectionSecurityConfig {
  ...
  authFailureHandling: {
    ...
    resetWindow: number; // time window to reset failure count (milliseconds)
    maxFailuresBeforeBlock: number; // max failures before temporary block
    blockDuration: number; // temporary block duration in milliseconds
  };
  ...
  securityMonitoring: {
    enabled: boolean;
    alertThresholds: {
      connectionFlood: number; // connections per minute from single IP
      authFailureRate: number; // auth failures per minute
      suspiciousPatterns: number; // suspicious pattern score threshold
    };
    monitoringInterval: number; // monitoring check interval in milliseconds
    alertCooldown: number; // cooldown between similar alerts in milliseconds
  };
...
export class ConnectionSecurityManager extends EventEmitter {
  ...
  private connectionsByServer = new Map<string, Set<string>>();
  
  // Authentication failure tracking
  private authFailures = new Map<string, AuthFailureRecord>(); // key: ip:serverId
  ...
  constructor(
    ...
    config: Partial<ConnectionSecurityConfig> = {}
  ) {
    super();
  ...
  registerConnection(
    ...
    // Log connection
    this.auditService.logger.logSuccess(
      'connection_registered',
      { connectionId, serverId, ip, userAgent },
      { serverId }
    );
  ...
  recordAuthenticationFailure(
    ...
    this.auditService.logger.logFailure(
   ...
   */
  private calculateNextAllowedAttempt(failureCount: number): Date {
    const delay = Math.min(
      this.config.authFailureHandling.baseDelay * Math.pow(this.config.authFailureHandling.backoffMultiplier, failureCount - 1),
      this.config.authFailureHandling.maxDelay
    );
   ...
   */
  private generateSecurityAlert(alert: Omit<SecurityAlert, 'id' | 'timestamp' | 'acknowledged'>): SecurityAlert {
    ...
    this.auditService.logger.logError(
      ...
      `Security alert: ${fullAlert.details.description}`,
      { serverId: fullAlert.source.serverId }
    );
  ...
  acknowledgeAlert(alertId: string, acknowledgedBy: string): boolean {
    ...
    this.auditService.logger.logSuccess(
      ...
      { serverId: alert.source.serverId }
    );
   ...
   */
  private createDefaultConfig(customConfig: Partial<ConnectionSecurityConfig>): ConnectionSecurityConfig {
    const defaultConfig: ConnectionSecurityConfig = {
      connectionLimits: {
        enabled: true,
   ...
   */
  private mergeConfig(base: ConnectionSecurityConfig, override: Partial<ConnectionSecurityConfig>): ConnectionSecurityConfig {
    const result = { ...base };

    for (const [key, value] of Object.entries(override)) {
      if (value && typeof value === 'object' && !Array.isArray(value)) {
        result[key as keyof ConnectionSecurityConfig] = {
          ...result[key as keyof ConnectionSecurityConfig],
          ...value
        } as any;
      } else {
        result[key as keyof ConnectionSecurityConfig] = value as any;
      }
    }
