...
export interface BanEntry {
  ...
  target: string; // playerId, IP address, or device ID
  targetName?: string; // player name if available
  reason: string;
  bannedBy: string;
  bannedAt: Date;
  expiresAt?: Date; // undefined means permanent ban
  isActive: boolean;
}
...
export class WhitelistManager {
  ...
  private banCache = new Map<string, BanEntry[]>();
  private pendingBanOperations = new Map<string, BanOperation[]>();
  private banSyncStatus = new Map<string, BanSyncStatus>();
   ...
   */
  async isWhitelisted(serverId: string, playerId: string): Promise<boolean> {
   ...
   */
  async getBanList(serverId: string): Promise<BanEntry[]> {
   ...
   */
  async isBanned(serverId: string, banType: 'player' | 'ip' | 'device', target: string): Promise<boolean> {
   ...
   */
  async getBanInfo(serverId: string, banType: 'player' | 'ip' | 'device', target: string): Promise<BanEntry | null> {
    const banList = await this.getBanList(serverId);
   ...
   */
  private async cacheBanOperation(serverId: string, operation: BanOperation): Promise<void> {
    const logger = this.ctx.logger('mochi-link:ban');
  ...
  private optimizeBanOperations(
    existing: BanOperation[], 
    newOperation: BanOperation
  ): BanOperation[] {
    const optimized = [...existing];
    
    // Find conflicting operations for the same target
    const conflictIndex = optimized.findIndex(op => 
      op.banType === newOperation.banType &&
    ...
    const duplicateIndex = optimized.findIndex(op => 
      op.banType === newOperation.banType &&
      op.target === newOperation.target && 
      op.type === newOperation.type
    );
   ...
   */
  private async executeBanOperationOnServer(
    serverId: string, 
    ...
    try {
      ...
      if (operation.type === 'ban') {
        return await bridge.banTarget(
          operation.banType,
          operation.target, 
          operation.targetName || operation.target, 
      ...
      this.ctx.logger('mochi-link:ban').error(
        `Failed to execute ${operation.type} operation on server ${serverId}:`, error
      );
      return false;
    }
  }
   ...
   */
  private async updateLocalBanList(serverId: string, operation: BanOperation): Promise<void> {
    let bans = this.banCache.get(serverId) || [];
    
    if (operation.type === 'ban') {
      // Remove existing ban for the same target if present
      bans = bans.filter(ban => 
        !(ban.banType === operation.banType && ban.target === operation.target)
      );
   ...
   */
  private async syncBansFromServer(serverId: string): Promise<void> {
    const logger = this.ctx.logger('mochi-link:ban');
    ...
    try {
      ...
      const entries: BanEntry[] = serverBans.map((ban: any) => ({
   ...
   * Update ban cache
   */
  private updateBanCache(serverId: string, bans: BanEntry[]): void {
    this.banCache.set(serverId, [...bans]);
  }
   ...
   */
  private updateBanSyncStatus(serverId: string, success: boolean, error?: string): void {
    const existing = this.banSyncStatus.get(serverId);
    const status: BanSyncStatus = {
      serverId,
      lastSync: new Date(),
   ...
   */
  async syncFromServer(serverId: string): Promise<void> {
    const logger = this.ctx.logger('mochi-link:whitelist');
    ...
    try {
      ...
      const entries: WhitelistEntry[] = serverWhitelist.map((entry: any) => ({
        playerId: entry.id,
   ...
   */
  async syncToServer(serverId: string): Promise<void> {
    const logger = this.ctx.logger('mochi-link:whitelist');
    ...
    try {
      ...
      const serverWhitelist = await bridge.getWhitelist();
      const serverPlayerIds = new Set(serverWhitelist.map((entry: any) => entry.id));
      
      // Find differences
      const toAdd = cache.entries.filter(entry => !serverPlayerIds.has(entry.playerId));
      const toRemove = serverWhitelist.filter((entry: any) => 
        !cache.entries.some(cacheEntry => cacheEntry.playerId === entry.id)
      );
      ...
      for (const entry of toRemove) {
        await bridge.removeFromWhitelist((entry as any).id);
        logger.debug(`Removed ${(entry as any).id} from server ${serverId} whitelist`);
      }
   ...
   */
  async forceSyncBidirectional(serverId: string): Promise<void> {
    const logger = this.ctx.logger('mochi-link:whitelist');
   ...
   */
  private async cacheOperation(serverId: string, operation: WhitelistOperation): Promise<void> {
    const logger = this.ctx.logger('mochi-link:whitelist');
  ...
  private optimizeOperations(
    existing: WhitelistOperation[], 
    newOperation: WhitelistOperation
  ): WhitelistOperation[] {
    const optimized = [...existing];
    
    // Find conflicting operations for the same player
    const conflictIndex = optimized.findIndex(op => 
      op.playerId === newOperation.playerId && op.type !== newOperation.type
    );
   ...
   */
  async processPendingOperations(serverId: string): Promise<void> {
    const logger = this.ctx.logger('mochi-link:whitelist');
   ...
   */
  private async executeOperationOnServer(
    serverId: string, 
    ...
    try {
      ...
      if (operation.type === 'add') {
        return await bridge.addToWhitelist(
          operation.playerId, 
      ...
      this.ctx.logger('mochi-link:whitelist').error(
        `Failed to execute ${operation.type} operation on server ${serverId}:`, error
      );
      return false;
    }
   ...
   */
  private async updateLocalWhitelist(serverId: string, operation: WhitelistOperation): Promise<void> {
    const cache = this.whitelistCache.get(serverId);
    if (!cache) {
      return;
    }
   ...
   * Update cache
   */
  private updateCache(serverId: string, entries: WhitelistEntry[]): void {
   ...
   * Update sync status
   */
  private updateSyncStatus(serverId: string, success: boolean, error?: string): void {
    const existing = this.syncStatus.get(serverId);
    const status: WhitelistSyncStatus = {
      serverId,
      lastSync: new Date(),
      pendingOperations: existing?.pendingOperations || 0,
      syncErrors: success ? [] : (error ? [error] : existing?.syncErrors || []),
   ...
   */
  private async processPendingBanOperations(serverId: string): Promise<void> {
    const logger = this.ctx.logger('mochi-link:ban');
  ...
  private async saveWhitelistToDatabase(serverId: string, entries: WhitelistEntry[]): Promise<void> {
    try {
  ...
  private async loadPendingOperationsFromDatabase(): Promise<void> {
    try {
      ...
      // Group by server
      const serverOps = new Map<string, WhitelistOperation[]>();
      ...
      const serverBanOps = new Map<string, BanOperation[]>();
  ...
  private async recordAuditLog(
    serverId: string, 
    operation: WhitelistOperation | BanOperation, 
    ...
    try {
      ...
      if ('banType' in operation) {
        (operationData as any).banType = operation.banType;
        (operationData as any).duration = operation.duration;
      }
  ...
  private async loadBansFromDatabase(serverId: string): Promise<BanEntry[]> {
    try {
      ...
      const latestBanState = banOps.sort((a, b) => 
   ...
   */
  async clearPendingOperations(serverId: string, executor: string): Promise<void> {
    this.pendingOperations.delete(serverId);
    await this.clearPendingOperationsFromDatabase(serverId);
    ...
    this.ctx.logger('mochi-link:whitelist').info(
      `Cleared all pending operations for server ${serverId} by ${executor}`
    );
  }
   ...
   */
  async clearPendingBanOperations(serverId: string, executor: string): Promise<void> {
    this.pendingBanOperations.delete(serverId);
    await this.clearPendingBanOperationsFromDatabase(serverId);
    ...
    this.ctx.logger('mochi-link:ban').info(
      `Cleared all pending ban operations for server ${serverId} by ${executor}`
    );
  }
  ...
  getBanStats(): {
    ...
    serversOnline: number;
    lastSyncErrors: number;
    bansByType: { player: number; ip: number; device: number };
  } {
  ...
  searchBans(query: string, serverId?: string): BanEntry[] {
    ...
    return allBans.filter(ban => 
      ban.target.toLowerCase().includes(lowerQuery) ||
